...
• Restoring force, $F$, is proportional to the displacement, $x$, but the force always acts back towards the equilibrium position: $$F ∝ −x$$
...
Another way to put it is: $$a ∝ -x$$• Where acceleration is proportional to the displacement, but in the opposite direction (always directed back towards the equilibrium position).

## Graphs

• Mass oscillating on a spring could be used with a marker pen to produce a record of the oscillation.
![[Pasted image 20231016223256.png]]

• The graph would appear as such:
![[Pasted image 20231016224531.png]]

• Using knowledge from [[A.2 Forces and Momentum]], velocity of oscillating mass at any moment can be calculated by determining the gradient of the displacement-time graph at that moment.
$$\text{velocity}, v = \frac{\text{change\ in\  displacement}}{\text{change\  in\  time}} = \frac{\delta x}{\delta t}$$
![[Pasted image 20231016224718.png]]

• Similarly, the acceleration at any time can be calculated.
$$\text{acceleration}, a = \frac{\text{change\ in\ velocity}}{\text{change\ in\ time}} = \frac{Δv}{Δt}$$
• The velocity graph has its maximum value, $v_0$ , when the displacement, $x$, is zero, and the velocity is zero when the displacement is at its maximum, $±x_0$ .

• Thus, acceleration can be shown as such.
![[Pasted image 20231016225602.png]]
...
All three graphs together:![[Pasted image 20231016225833.png]]

## The Simple Pendulum

**Equilibrium Position**
![[Pasted image 20231021183224.png]]
**Features**
• Consists of:
Small mass
Inextensible string
...
• pendulum oscillates because there is always a force acting toward the centre, as shown in Figure 1
![[Pasted image 20231016230320.png]]
...
- The restoring force that pulls the bob back to the equilibrium position is $$-mg\space\sin(θ) = ma = F$$
...
- This can be shown in $$a = -ω^2x$$
- $x$ = displacement
- $-w^2$ = angular frequency
- Due to the equation for displacement at a given time, it can potentially also be written as $$a = -ω^2x_0 \cos(ωt) = -ω^2$$

## Calculating Displacement & Velocity

- $x = x_0 \cos(ωt)$ for simple harmonic motion that begins at the extremes.
- $x = x_0 \sin(ωt)$ for simple harmonic motion that begins at the centre.
- $x = x_0 \sin(ωt+\phi)$ can be used for a different phase angle.
...
- Velocity (x unknown): $$v = ωx_0\space \cos(ωt)$$
- Velocity (t unknown): $$v = ±ω \sqrt {x_0^2-x^2}$$
- For where x is unknown, if velocity wants to be found at a different phase angle, the following can be used: $$v=ωx_0​\space\cos(ωt+ϕ)$$
...
1. Mass
2. Spring
   ![[Pasted image 20231024003557.png]]
   **Hooke's Law**
...
- The general formula for kinetic energy is:
  $$ E_K = \frac12mv^2$$
- And the equation for velocity is: $$v = ±ω \sqrt {x_0^2-x^2}$$
- It can be substituted to make: $$E_K = \frac12mω(x_0^2-x^2)$$
...
- In addition, since v can also be: $$v = ωx_0\space \cos(ωt)$$
- Kinetic energy can be alternatively expressed as: $$E_K = \frac{1}{2}mw^2x^2_0\cos^2(\omega t)$$
- This can be graphically represented as:![[Pasted image 20231023054212.png]]
...
- This can be graphically represented as: ![[Pasted image 20231023054355.png]]
...
- $$E_P = (\frac{1}{2}mw^2x^2_0) - (\frac12mω(x_0^2-x^2))$$
- This can be further simplified as: $$E_P = \frac{1}{2}mw^2x^2$$
- This can be graphically represented as: ![[Pasted image 20231023180331.png]]
...
- $\omega$ = angular frequency $$ω = 2πf  = \frac{2π}{T}$$
- $1\space rad =\frac{180°}{π}(= 57.3°)$
...
1. ω = 2πf
   ...
   In your case, the frequency (f) is given as 196 Hz. So, you can calculate ω as follows:
...
- The angle that we start at is arbitrary, but the change stays constant relative to the size of the wave.
  ![[Pasted image 20231023022334.png]]
...
- In the picture, the phase difference $\phi$ is 1 rad OR $0.16\degree$
  ![[Pasted image 20231023020137.png]]
  ![[Pasted image 20231023031217.png]]
