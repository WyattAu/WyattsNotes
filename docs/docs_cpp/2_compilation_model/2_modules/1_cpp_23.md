---
title: C++23
description: "C++: C++23 — 1. The Primary Module Interface Unit (PMIU); Syntax and Structure; File Extensions; 2. Module Implementation Units (MIU)."
...
2. **Global Module Fragment** (for legacy macros that cannot be converted):
...
3. **Predefined macros** (compiler-defined macros like `__FILE__`, `__LINE__`, `NDEBUG`) are always
...
<Tabs>
  <TabItem value="clang" label="Clang (16+)">
...
```
  ...
  <TabItem value="gcc" label="GCC (12+)">
...
```
  ...
  <TabItem value="msvc" label="MSVC (VS 2022 17.4+)">
...
consumers cannot see its symbols — and the error message from the compiler is typically cryptic.
