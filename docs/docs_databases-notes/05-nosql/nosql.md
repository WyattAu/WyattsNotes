...
$$\text{PA} \to \text{EL} : \text{when no partition, prefer availability and latency over consistency}$$

$$\text{PC} \to \text{EC} : \text{when no partition, prefer consistency, accepting higher latency}$$
