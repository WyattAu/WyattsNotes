...
description: "University-level notes on Solid State Physics: 1. Crystal Structures; 1.1 Lattices and Basis; 1.2 Bravais Lattices; 1.3 Common Crystal Structures."
...
- For a fixed wavelength and a single crystal, very few reciprocal lattice points lie on
...
:::caution
Common Pitfall
The effective mass is a tensor quantity . For crystals with cubic symmetry, it reduces to
...
Frequencies within a few percent) but is less reliable for band gaps (LDA underestimates
...
Modifying the band structure. Tensile strain reduces the band gap, while compressive
