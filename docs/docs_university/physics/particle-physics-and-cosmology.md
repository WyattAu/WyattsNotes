...
$\mathbf{'\{'}x{'\}'} \to -\mathbf{'\{'}x{'\}'}$. Under parity, polar vectors change sign while
axial vectors do not.
...
$\phi \to \phi + \delta\phi$. If the action $S = \int \mathcal{'\{'}L{'\}'}\,d^4x$ is invariant,
then the Current
...
$\mathcal{'\{'}M{'\}'} = 0$. This is the expected result: identical Fermions cannot be distinguished
in the final state at $90^\circ$ scattering.
...
When squaring, there is a cross term $\mathcal{'\{'}M{'\}'}_s\mathcal{'\{'}M{'\}'}_t^*$ That leads
to interference between the two diagrams. This interference is destructive At small angles (forward
...
**Quark colour states:** $q \in \mathbf{'\{'}3{'\}'}$ (triplet),
$\bar{q} \in \bar{\mathbf{'\{'}3{'\}'}}$ (antitriplet).
...
The tensor product $\mathbf{'\{'}3{'\}'} \otimes \bar{\mathbf{'\{'}3{'\}'}}$ can be decomposed using
the Clebsch--Gordan series for SU(3):
...
Baryons. In the full baryon wavefunction, the colour part is antisymmetric, so the Combined flavour
$\otimes$ spin $\otimes$ space part must be symmetric (for ground-state Baryons, $L = 0$So the space
part is symmetric).
