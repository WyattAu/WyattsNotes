...
$$\mathrm◆LB◆New \to \mathrm◆LB◆Ready \to \mathrm◆LB◆Running \to \mathrm◆LB◆Terminated$$

$$\mathrm◆LB◆Running \to \mathrm◆LB◆Blocked \to \mathrm◆LB◆Ready$$
...
- **CPU utilisation:** Keep the CPU busy ($\mathrm◆LB◆utilisation = 1 - p$ where $p$ is the I/O wait
  probability).
...
Averaging: $\tau_◆LB◆n+1◆RB◆ = \alpha t_n + (1 - \alpha) \tau_n$.
...
- **Maximum:** Matrix $\mathrm◆LB◆Max$ where $\mathrm◆LB◆Max[i][j]$ is the maximum demand of process $i$
  for resource $j$.
- **Allocation:** Matrix $\mathrm◆LB◆Alloc$ where $\mathrm◆LB◆Alloc[i][j]$ is the current allocation.
- **Need:** $\mathrm◆LB◆Need[i][j] = \mathrm◆LB◆Max[i][j] - \mathrm◆LB◆Alloc[i][j]$.
...
Safety check: $P_1$ has $\mathrm◆LB◆Need = (1,2,2) \leq (3,3,2) = A$. Execute $P_1$Release $(2,0,0)$New
$A = (5,3,2)$. Then $P_3$: $\mathrm◆LB◆Need = (0,1,1) \leq (5,3,2)$. Continuing, All processes can
...
_Step 1:_ Verify $\mathrm◆LB◆Request_1 = (1,0,2) \leq \mathrm◆LB◆Need_1 = (1,2,2)$. OK.

_Step 2:_ Verify $\mathrm◆LB◆Request_1 = (1,0,2) \leq A = (3,3,2)$. OK.
...
1. $P_1$: $\mathrm◆LB◆Need = (0,2,0) \leq (2,3,0)$. Execute, $A = (2,3,0) + (3,0,2) = (5,3,2)$.
2. $P_3$: $\mathrm◆LB◆Need = (0,1,1) \leq (5,3,2)$. Execute, $A = (5,3,2) + (2,1,1) = (7,4,3)$.
3. $P_4$: $\mathrm◆LB◆Need = (4,3,1) \leq (7,4,3)$. Execute, $A = (7,4,3) + (0,0,2) = (7,4,5)$.
4. $P_0$: $\mathrm◆LB◆Need = (7,4,3) \leq (7,4,5)$. Execute, $A = (7,4,5) + (0,1,0) = (7,5,5)$.
5. $P_2$: $\mathrm◆LB◆Need = (6,0,0) \leq (7,5,5)$. Execute, $A = (7,5,5) + (3,0,2) = (10,5,7)$.
...
_Step 1:_ $\mathrm◆LB◆Request_0 = (0,2,0) \leq \mathrm◆LB◆Need_0 = (7,4,3)$. OK.
...
$\mathrm◆LB◆Request_0 = 4$$\mathrm◆LB◆Request_1 = 2$$\mathrm◆LB◆Request_2 = 5$.

_Detection:_

1. Work = 1. No process has $\mathrm◆LB◆Request \leq 1$.
...
proceed ($\mathrm◆LB◆Request_1 = 2 \leq 4$). After $P_1$ finishes, Available = $4 + 2 = 6$. $P_0$:
$\mathrm◆LB◆Request_0 = 4 \leq 6$Proceeds. After: Available = $6 + 5 = 11$. $P_2$ proceeds. Deadlock
resolved.
...
$$\mathrm◆LB◆physical address = \mathrm◆LB◆PT[p] \times F + f$$

Where $\mathrm◆LB◆PT[p]$ is the frame number for page $p$ and $F$ is the frame size.
...
$$\mathrm◆LB◆Address:  \underbrace◆LB◆p_1◆RB◆_◆LB◆\mathrm◆LB◆outer◆RB◆ \mid \underbrace◆LB◆p_2◆RB◆_◆LB◆\mathrm◆LB◆inner◆RB◆ \mid \underbrace◆LB◆d◆RB◆_◆LB◆\mathrm◆LB◆offset◆RB◆$$
...
$$\mathrm◆LB◆physical address = \mathrm◆LB◆base + \mathrm◆LB◆offset, \quad \mathrm◆LB◆if  \mathrm◆LB◆offset \lt \mathrm◆LB◆limit$$
...
$$\mathrm◆LB◆Address:  \underbrace◆LB◆s◆RB◆_◆LB◆\mathrm◆LB◆segment◆RB◆ \mid \underbrace◆LB◆p◆RB◆_◆LB◆\mathrm◆LB◆page◆RB◆ \mid \underbrace◆LB◆d◆RB◆_◆LB◆\mathrm◆LB◆offset◆RB◆$$
...
$$\mathrm◆LB◆EAT = (1 - p) \times \mathrm◆LB◆ma + p \times \mathrm◆LB◆pf$$
...
$$\mathrm◆LB◆EAT = h \times (\mathrm◆LB◆TLB + \mathrm◆LB◆ma) + (1 - h) \times (\mathrm◆LB◆TLB + \mathrm◆LB◆ma + \mathrm◆LB◆ma)$$
...
$\mathrm◆LB◆TLB + \mathrm◆LB◆ma = 2 + 100 = 102$ ns.

_Case 2: TLB miss, no page fault_ ($0.20 \times 0.9995 = 0.1999$): Time =
$\mathrm◆LB◆TLB + 2 \times \mathrm◆LB◆ma = 2 + 200 = 202$ ns.

_Case 3: Page fault_ ($0.0005$): Time =
$\mathrm◆LB◆TLB + \mathrm◆LB◆ma + \mathrm◆LB◆pf = 2 + 100 + 8 \times 10^6 = 8000002$ ns.

$$\mathrm◆LB◆EAT = 0.7996 \times 102 + 0.1999 \times 202 + 0.0005 \times 8\,000\,102$$
$$\mathrm◆LB◆EAT = 81.56 + 40.38 + 4000.05 = 4121.99 \;\mathrm◆LB◆ns \approx 4.12 \;\mu\mathrm◆LB◆s$$
...
_Proof._ By the pigeonhole principle, if $\sum_◆LB◆i=1◆RB◆^◆LB◆n◆RB◆ \lvert W_i \rvert \gt F$ where $F$ is the
...
Maximum file size:
$48\;\mathrm◆LB◆KiB + 4\;\mathrm◆LB◆MiB + 4\;\mathrm◆LB◆GiB + 4\;\mathrm◆LB◆TiB \approx 4\;\mathrm◆LB◆TiB$.
...
**Mean time to failure (MTTF).** If each disk has $\mathrm◆LB◆MTTF_◆LB◆\mathrm◆LB◆disk◆RB◆$ and the MTTR (mean
time to repair) is $T_◆LB◆\mathrm◆LB◆repair◆RB◆$:

$$\mathrm◆LB◆MTTF_◆LB◆\mathrm◆LB◆RAID\;5◆RB◆ \approx \frac◆LB◆\mathrm◆LB◆MTTF_◆LB◆\mathrm◆LB◆disk◆RB◆^2◆RB◆◆LB◆n(n-1) \cdot T_◆LB◆\mathrm◆LB◆repair◆RB◆◆RB◆$$
...
Eight 2 TiB disks. $\mathrm◆LB◆MTTF_◆LB◆\mathrm◆LB◆disk◆RB◆ = 1.2 \times 10^6$ hours, $T_◆LB◆\mathrm◆LB◆repair◆RB◆ = 24$
hours.

_RAID 0:_ Capacity = $8 \times 2 = 16$ TiB. No fault tolerance. MTTF =
$\mathrm◆LB◆MTTF_◆LB◆\mathrm◆LB◆disk◆RB◆ / 8 = 150000$ hours.

_RAID 5:_ Capacity = $7 \times 2 = 14$ TiB.
$\mathrm◆LB◆MTTF_◆LB◆\mathrm◆LB◆RAID\;5◆RB◆ \approx \frac◆LB◆(1.2 \times 10^6)^2◆RB◆◆LB◆8 \times 7 \times 24◆RB◆ = \frac◆LB◆1.44 \times 10^◆LB◆12◆RB◆◆RB◆◆LB◆1344◆RB◆ \approx 1.07 \times 10^9$
hours.
...
- Entropy: 22--28 bits on 64-bit systems, providing $2^◆LB◆22◆RB◆$ to $2^◆LB◆28◆RB◆$ possible layouts.
...
Total message-passing time per second: $10^6 \times 2 \;\mu\mathrm◆LB◆s = 2$ seconds of CPU time per
...
1. $\mathrm◆LB◆Request_0 = (1,0,0) \leq \mathrm◆LB◆Need_0 = (2,2,1)$. OK.
2. $\mathrm◆LB◆Request_0 = (1,0,0) \leq A = (3,2,1)$. OK.
3. Pretend to allocate: $A = (2,2,1)$$\mathrm◆LB◆Alloc_0 = (2,0,0)$$\mathrm◆LB◆Need_0 = (1,2,1)$.
4. Safety: Work = $(2,2,1)$.
...
(a) Offset = $\log_2(4096) = 12$ bits. Page number = $32 - 12 = 20$ bits. (b) $2^◆LB◆20◆RB◆ = 1048576$
...
$\mathrm◆LB◆EAT = 0.8991 \times 102 + 0.0999 \times 202 + 0.001 \times 6000002$
...
Assume no TLB for simplicity: $\mathrm◆LB◆EAT = (1 - p) \times 100 + p \times 8 \times 10^6$.

$200 = 100 - 100p + 8 \times 10^6 p$ $100 = 7999900 p$ $p \approx 1.25 \times 10^◆LB◆-5◆RB◆$
...
Average waiting time = $\frac◆LB◆6 + 4 + 7◆RB◆◆LB◆3◆RB◆ = 5.67$ ms.
