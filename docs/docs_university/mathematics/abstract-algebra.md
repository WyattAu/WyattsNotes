...
**Example 4.** $(\mathbb{{'}Z{}'}/n\mathbb{{'}Z{}'}, +)$ is an abelian group of order $n$Called the
**cyclic group** of order $n$.

**Example 5.** The set $GL_n(\mathbb{{'}R{}'})$ of all $n \times n$ invertible real matrices, under
matrix multiplication, forms the **general linear group**.
...
rotation subgroup $\langle r \rangle \cong \mathbb{{'}Z{}'}/4\mathbb{{'}Z{}'}$ of order $4$Four
subgroups of order $2$ generated By reflections, and $\langle r^2, s \rangle \cong V_4$ (the Klein
four-group).
...
By Theorem 2.4, every subgroup of $\mathbb{{'}Z{}'}/12\mathbb{{'}Z{}'}$ is cyclic, and there is
exactly one subgroup Of order $d$ for each divisor $d$ of $12$.
...
**Example.** The set $SL_n(\mathbb{{'}R{}'})$ of $n \times n$ real matrices with determinant $1$ is
...
**Problem.** Let $G = (\mathbb{{'}Z{}'}, +)$ and $H = \{5k - 3m : k, m \in \mathbb{{'}Z{}'}\}$. Show
that $H = \mathbb{{'}Z{}'}$.
...
**Problem.** Let $H = \langle 4 \rangle \leq \mathbb{{'}Z{}'}/12\mathbb{{'}Z{}'}$. Find all cosets
of $H$.
...
$|\mathbb{{'}Z{}'}/12\mathbb{{'}Z{}'}| = 12$ So $[\mathbb{{'}Z{}'}/12\mathbb{{'}Z{}'} : H] = 4$. The
cosets are:
...
**Example.** $\mathbb{{'}Z{}'}/n\mathbb{{'}Z{}'}$ is the quotient of $\mathbb{{'}Z{}'}$ by
$n\mathbb{{'}Z{}'}$.
...
$r \in \mathbb{{'}R{}'}^*$The matrix $\begin{pmatrix} r & 0 \\ 0 & 1 \end{pmatrix}$ has determinant
$r$.
...
_Solution._ Define $\phi : \mathbb{{'}C{}'}^* \to \mathbb{{'}R{}'}^+$ by $\phi(z) = |z|$. This is a
homomorphism since $|zw| = |z||w|$. It is surjective since for any $r > 0$ $\phi(r) = r$. The kernel
is $\ker(\phi) = \{z \in \mathbb{{'}C{}'}^* : |z| = 1\} = S^1$The unit circle. By the first
isomorphism theorem, $\mathbb{{'}C{}'}^* / S^1 \cong \mathbb{{'}R{}'}^+$. $\blacksquare$
...
$\{x \in \mathbb{{'}R{}'}^* : \mathrm{sgn{}(x) = 1\} = \mathbb{{'}R{}'}^+$. By the first isomorphism
theorem, $\mathbb{{'}R{}'}^* / \mathbb{{'}R{}'}^+ \cong \mathbb{{'}Z{}'}/2\mathbb{{'}Z{}'}$.
$\blacksquare$
...
$\det : GL_n(\mathbb{{'}R{}'}) \to \mathbb{{'}R{}'}^*$ Is a surjective group homomorphism with
kernel $SL_n(\mathbb{{'}R{}'})$. By the first isomorphism theorem,
...
$\phi(k) = k + n\mathbb{{'}Z{}'}$. Verify that $\phi$ is a surjective homomorphism and identify the
quotient.
...
$\phi(k)$. The kernel is
$\ker(\phi) = \{k \in \mathbb{{'}Z{}'} : k \in n\mathbb{{'}Z{}'}\} = n\mathbb{{'}Z{}'}$. By the
first isomorphism theorem,
$\mathbb{{'}Z{}'}/n\mathbb{{'}Z{}'} \cong \mathbb{{'}Z{}'}/n\mathbb{{'}Z{}'}$ (the canonical
...
group of units Modulo $n$. An automorphism of $\mathbb{{'}Z{}'}/n\mathbb{{'}Z{}'}$ is determined by
where it sends $1$And $1$ can map to any generator, i.e., any $[k]$ with $\gcd(k, n) = 1$.
...
**Example.** $D_n = \mathbb{{'}Z{}'}/n\mathbb{{'}Z{}'} \rtimes \mathbb{{'}Z{}'}/2\mathbb{{'}Z{}'}$
where the action of $\mathbb{{'}Z{}'}/2\mathbb{{'}Z{}'}$ on $\mathbb{{'}Z{}'}/n\mathbb{{'}Z{}'}$
sends $k \mapsto -k$ (inversion). Explicitly: $(a, 0)(b, 0) = (a + b, 0)$$(a, 0)(b, 1) = (a + b, 1)$
...
The action of a Sylow $3$-subgroup $K = \langle b \rangle \cong \mathbb{{'}Z{}'}/3\mathbb{{'}Z{}'}$
on $H$ by conjugation Is a homomorphism
$\mathbb{{'}Z{}'}/3\mathbb{{'}Z{}'} \to \mathrm{Aut{}(\mathbb{{'}Z{}'}/7\mathbb{{'}Z{}'}) \cong \mathbb{{'}Z{}'}/6\mathbb{{'}Z{}'}$.

$\mathrm{Aut{}(\mathbb{{'}Z{}'}/7\mathbb{{'}Z{}'})$ has exactly one subgroup of order $3$ (generated
by $x \mapsto 2x$ Since $2^3 = 8 \equiv 1 \pmod{7}$).
...
**Example 2.** $\mathbb{{'}Z{}'}/n\mathbb{{'}Z{}'}$ is a commutative ring with unity. It is a field
if and only if $n$ is prime.

**Example 3.** $\mathbb{{'}Q{}'}$$\mathbb{{'}R{}'}$$\mathbb{{'}C{}'}$ are fields.

**Example 4.** The set $M_n(\mathbb{{'}R{}'})$ of $n \times n$ real matrices is a non-commutative
ring with unity.

**Example 5.** $\mathbb{{'}Z{}'}[i] = \{a + bi : a, b \in \mathbb{{'}Z{}'}\}$ (Gaussian integers) is
...
**Proposition 8.3.** $\mathbb{{'}Z{}'}/n\mathbb{{'}Z{}'}$ is an integral domain if and only if $n$
is prime.
...
**Example.** $\mathbb{{'}Q{}'}(\sqrt{2}) = \{a + b\sqrt{2} : a, b \in \mathbb{{'}Q{}'}\}$ is a
field. The inverse of $a + b\sqrt{2}$ (with $a, b$ not both zero) is
$\frac{a - b\sqrt{2}}{a^2 - 2b^2}$.

**Example.** For any prime $p$$\mathbb{{'}Z{}'}/p\mathbb{{'}Z{}'}$ is a field with $p$ elements,
denoted $\mathbb{{'}F{}'}_p$.
...
**Example.** The map $\phi : \mathbb{{'}Z{}'} \to \mathbb{{'}Z{}'}/n\mathbb{{'}Z{}'}$ given by
$\phi(k) = [k]$ is a surjective Ring homomorphism with kernel $n\mathbb{{'}Z{}'}$.

**Example.** The evaluation map $\phi : \mathbb{{'}R{}'}[x] \to \mathbb{{'}R{}'}$ given by
$\phi(f) = f(a)$ for a fixed $a \in \mathbb{{'}R{}'}$ is a surjective ring homomorphism with kernel
...
$\phi(a + bi) = (a + 2b) + 5\mathbb{{'}Z{}'}$. Show that $\phi$ is a surjective ring homomorphism
and find its kernel.
...
These differ by $5bd + 5\mathbb{{'}Z{}'} = 0 + 5\mathbb{{'}Z{}'}$ since $5bd$ is a multiple of $5$.
So $\phi$ preserves Multiplication. Additivity is clear. Also $\phi(1) = 1 + 5\mathbb{{'}Z{}'}$. ✓
...
$\ker(\phi) = (1 + 2i)$ (the principal ideal generated by $1 + 2i$ in $\mathbb{{'}Z{}'}[i]$). By the
ring isomorphism theorem, $\mathbb{{'}Z{}'}[i]/(1+2i) \cong \mathbb{{'}Z{}'}/5\mathbb{{'}Z{}'}$.
$\blacksquare$

</details>

**Problem.** Let $R = \mathbb{{'}Z{}'}[x]/(x^2 - x)$. Describe the elements of $R$ and show that $R$
has zero divisors.
...
prime. The ideal $(6)$ is neither prime nor maximal. The ideal $(0)$ is prime ($\mathbb{{'}Z{}'}$ is
an integral domain) But not maximal ($\mathbb{{'}Z{}'}/(0) \cong \mathbb{{'}Z{}'}$ is not a field).
...
_Solution._ $\mathbb{{'}Z{}'}/(2) \cong \mathbb{{'}Z{}'}/2\mathbb{{'}Z{}'}$ is a field, so $(2)$ is
maximal by Theorem 9.3.
...
$f(x) = a_n x^n + \cdots + a_1 x + a_0 \in \mathbb{{'}Z{}'}[x]$. If there exists a prime $p$ such
that:
...
$\bar{f} = \bar{g}\bar{h} = \bar{a}_n x^n$ in $(\mathbb{{'}Z{}'}/p\mathbb{{'}Z{}'})[x]$. Since
$\mathbb{{'}Z{}'}/p\mathbb{{'}Z{}'}[x]$ is an integral domain, $\bar{g} = bx^r$ And $\bar{h} = cx^s$
for some $b, c \in \mathbb{{'}Z{}'}/p\mathbb{{'}Z{}'}$. In particular, the constant terms of $g$ and
$h$ are both divisible by $p$. But then $p^2$ divides $a_0$Contradicting condition (3).
$\blacksquare$
...
$a, b \in \mathbb{{'}R{}'}$Then $a + b = 0$ and $ab = 1$Giving $-a^2 = 1$Which has no real solution.
...
$\mathbb{{'}F{}'}_2$. The only irreducible quadratic in $\mathbb{{'}F{}'}_2[x]$ is $x^2 + x + 1$.
Division gives $x^5 + x + 1 = (x^2+x+1)(x^3 + x^2) + 1$So $x^2 + x + 1$ does not divide $\bar{f}$.
...
_Proof (existence)._ Consider the splitting field of $f(x) = x^{p^n} - x$ over $\mathbb{{'}F{}'}_p$.
...
$x \in \mathbb{{'}F{}'}_{p^n}^*$Meaning every element is a root of $x^m - 1$. Since $x^m - 1$ has at
most $m$ roots in a field, $p^n - 1 \leq m$. But $m$ divides $p^n - 1$ So $m = p^n - 1$.
$\blacksquare$
...
**Problem.** Compute $[\mathbb{{'}Q{}'}(\sqrt{2}, \sqrt{3}) : \mathbb{{'}Q{}'}]$ and find the
...
$\mathbb{{'}Q{}'}$. So $[\mathbb{{'}Q{}'}(\sqrt{2}, \sqrt{3}) : \mathbb{{'}Q{}'}(\sqrt{2})] = 2$.
...
$|\mathrm{Aut{}(\mathbb{{'}Q{}'}(\sqrt[4]{2})/\mathbb{{'}Q{}'})| = 2 < 4$. The extension is not
Galois. $\blacksquare$
...
**Theorem 13.2.** A polynomial $f \in \mathbb{{'}Q{}'}[x]$ is solvable by radicals if and only if
...
$\mathbb{{'}Z{}'}/p\mathbb{{'}Z{}'}$ acts on $X$ by cyclic permutation. Orbits have size $1$ or $p$.
An orbit has size $1$ precisely when $(g, g, \ldots, g) \in X$I.e., $g^p = e$. Since
...
So there are exactly two groups of order $4$: $\mathbb{{'}Z{}'}/4\mathbb{{'}Z{}'}$ and $V_4$.
$\blacksquare$
...
$q \in \mathbb{{'}Z{}'}[\sqrt{2}]$ With $|\alpha/\beta - q| \lt{} 1$ in both coordinates (round to
nearest integers). Then $\alpha = q\beta + r$ where $r = \alpha - q\beta$. We have
...
With $\tau\sigma\tau^{-1} = \sigma^{-1}$We get $\mathrm{Gal{}(E/\mathbb{{'}Q{}'}) \cong D_8$ (the
dihedral Group of order 8). $\blacksquare$
...
**$n_3 = 1$:** The Sylow $3$-subgroup $P_3 \cong \mathbb{{'}Z{}'}/3\mathbb{{'}Z{}'}$ is normal. $G$
is a semidirect product $\mathbb{{'}Z{}'}/3\mathbb{{'}Z{}'} \rtimes K$ where $K$ is a Sylow
$2$-subgroup ($\mathbb{{'}Z{}'}/4\mathbb{{'}Z{}'}$ or $V_4$). Computing the possible actions gives:
...
$\mathbb{{'}Z{}'}/3\mathbb{{'}Z{}'} \rtimes \mathbb{{'}Z{}'}/4\mathbb{{'}Z{}'}$ (the dicyclic group
of order $12$).
...
$\mathbb{{'}Z{}'}/4\mathbb{{'}Z{}'}$ Has no element of order $2$ besides its unique subgroup...
Actually, the argument is more subtle). This gives $A_4$.
...
:::caution Common Pitfall Not every UFD is a PID. The classic example is $\mathbb{{'}Z{}'}[x]$: the
ideal $(2, x)$ is not principal, But $\mathbb{{'}Z{}'}[x]$ is a UFD (since $\mathbb{{'}Z{}'}$ is a
UFD). :::
...
$x^3 - 3x + 1$ over $\mathbb{{'}Q{}'}$ is $A_3 \cong \mathbb{{'}Z{}'}/3\mathbb{{'}Z{}'}$ (the
discriminant is a square). :::

:::caution Common Pitfall A field extension can be algebraic without being finite. For example,
$\overline{\mathbb{{'}Q{}'}}/\mathbb{{'}Q{}'}$ (algebraic closure of $\mathbb{{'}Q{}'}$) is
algebraic but infinite-dimensional. :::
...
**Problem 4.** Find all subgroups of $\mathbb{{'}Z{}'}/12\mathbb{{'}Z{}'}$ and draw the subgroup
lattice.

<details>
<summary>Solution</summary>

_Solution._ By Theorem 2.4, every subgroup of the cyclic group $\mathbb{{'}Z{}'}/12\mathbb{{'}Z{}'}$
is cyclic, And there is exactly one subgroup of order $d$ for each divisor $d$ of $12$.
...
**Problem 8.** Let $\phi : \mathbb{{'}Z{}'} \to \mathbb{{'}Z{}'}$ be defined by $\phi(n) = 3n$.
Determine whether $\phi$ is a group homomorphism, find its kernel and image, and explain why it is
...
$Q \cong \mathbb{{'}Z{}'}/5\mathbb{{'}Z{}'}$ are normal. Since $P \cap Q = \{e\}$ (their orders are
coprime) And $|PQ| = |P||Q|/|P \cap Q| = 15 = |G|$We have $G = PQ$. Since both are normal with
...
semidirect product $\mathbb{{'}Z{}'}/7\mathbb{{'}Z{}'} \rtimes \mathbb{{'}Z{}'}/3\mathbb{{'}Z{}'}$
...
_Solution._ By Theorem 9.3, $(p)$ is prime in $\mathbb{{'}Z{}'}$ iff $\mathbb{{'}Z{}'}/(p)$ is an
integral domain. $\mathbb{{'}Z{}'}/(2) \cong \mathbb{{'}Z{}'}/2\mathbb{{'}Z{}'}$ is a field, hence
an integral domain, so $(2)$ is prime.

$\mathbb{{'}Z{}'}/(4)$: we have $[2][2] = [4] = [0]$ but $[2] \neq [0]$. So $\mathbb{{'}Z{}'}/(4)$
has zero divisors and is not an integral domain. Therefore $(4)$ is not prime. $\blacksquare$
...
$a, b \in \mathbb{{'}R{}'}$. Then $a + b = 0$ and $ab = 1$So $-a^2 = 1$Giving $a^2 = -1$Which has no
real solution. Thus $x^2 + 1$ is irreducible in $\mathbb{{'}R{}'}[x]$.
...
_Solution._ **UFD:** By Gauss's lemma, since $\mathbb{{'}Z{}'}$ is a UFD, $\mathbb{{'}Z{}'}[x]$ is a
UFD.
...
constant term). Contradiction. Therefore $(2, x)$ is not principal, and $\mathbb{{'}Z{}'}[x]$ is not
a PID. $\blacksquare$
...
**Problem 19.** Compute $[\mathbb{{'}Q{}'}(\sqrt{2}, \sqrt{3}) : \mathbb{{'}Q{}'}]$ and find the
Galois group.
...
irreducible over $\mathbb{{'}Q{}'}$ (by Eisenstein with $p = 2$). Then
$\sqrt{3} \notin \mathbb{{'}Q{}'}(\sqrt{2})$ (if $\sqrt{3} = a + b\sqrt{2}$ With
$a, b \in \mathbb{{'}Q{}'}$Squaring gives $3 = a^2 + 2b^2 + 2ab\sqrt{2}$Forcing $ab = 0$ and leading
to contradiction). So $[\mathbb{{'}Q{}'}(\sqrt{2}, \sqrt{3}) : \mathbb{{'}Q{}'}(\sqrt{2})] = 2$.
...
_Solution._ Define $\phi : \mathbb{{'}Z{}'}[x] \to \mathbb{{'}Z{}'}[i]$ by $\phi(f(x)) = f(i)$. This
is a ring Homomorphism (evaluation at $i$). It is surjective: any $a + bi \in \mathbb{{'}Z{}'}[i]$
equals $\phi(a + bx)$.

The kernel consists of polynomials $f \in \mathbb{{'}Z{}'}[x]$ with $f(i) = 0$. Since $x^2 + 1$ is
the minimal Polynomial of $i$ over $\mathbb{{'}Q{}'}$Every such $f$ is divisible by $x^2 + 1$ in
$\mathbb{{'}Q{}'}[x]$. By Gauss's lemma, $f$ is divisible by $x^2 + 1$ in $\mathbb{{'}Z{}'}[x]$ as
well. So $\ker(\phi) = (x^2 + 1)$.
...
Note: $\mathbb{{'}F{}'}_4$ is NOT a subfield of $\mathbb{{'}F{}'}_8$ (since $4$ does not divide
$8$), but $\mathbb{{'}F{}'}_4$ IS A subfield of $\mathbb{{'}F{}'}_{16}$ (since $4$ divides $16$).
More generally, $\mathbb{{'}F{}'}_{p^m} \subseteq \mathbb{{'}F{}'}_{p^n}$ If and only if $m$ divides
$n$. $\blacksquare$
