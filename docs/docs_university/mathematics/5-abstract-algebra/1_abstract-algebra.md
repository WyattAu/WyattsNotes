...
$\mathrm◆LB◆sgn◆RB◆(\sigma) = (-1)^◆LB◆N(\sigma)◆RB◆$ Where $N(\sigma)$ is the number of inversions of $\sigma$Is
a well-defined group homomorphism $\mathrm◆LB◆sgn◆RB◆ : S_n \to \{1, -1\}$. Its kernel is the **alternating
group** $A_n$ of even permutations.

_Proof._ We first show $\mathrm◆LB◆sgn◆RB◆$ is a homomorphism. For any transposition $\tau$ and any
Permutation $\sigma$Multiplying by $\tau$ changes the number of inversions by an odd number, So
$\mathrm◆LB◆sgn◆RB◆(\tau\sigma) = -\mathrm◆LB◆sgn◆RB◆(\sigma)$. For any $\sigma, \rho \in S_n$Write $\rho$ As a
product of $k$ transpositions. Then
$\mathrm◆LB◆sgn◆RB◆(\sigma\rho) = (-1)^k\mathrm◆LB◆sgn◆RB◆(\sigma) = \mathrm◆LB◆sgn◆RB◆(\sigma)\mathrm◆LB◆sgn◆RB◆(\rho)$. The
...
The **center** of $G$ is $Z(G) = \{z \in G : zg = gz \mathrm◆LB◆\ for\ all\ ◆RB◆ g \in G\}$.
...
The **image** of $\phi$ is $\mathrm◆LB◆im◆RB◆(\phi) = \{\phi(g) : g \in G\}$.

**Theorem 5.2.** $\ker(\phi) \trianglelefteq G$ and $\mathrm◆LB◆im◆RB◆(\phi) \leq H$.
...
$\mathrm◆LB◆im◆RB◆(\phi) \leq H$ is straightforward. $\blacksquare$
...
More generally (even if $\phi$ is not surjective), $G / \ker(\phi) \cong \mathrm◆LB◆im◆RB◆(\phi)$.
...
By construction, $\mathrm◆LB◆im◆RB◆(\overline◆LB◆\phi◆RB◆) = \mathrm◆LB◆im◆RB◆(\phi)$. $\blacksquare$
...
$\phi(x) = \mathrm◆LB◆sgn◆RB◆(x)$. This is a homomorphism since
$\mathrm◆LB◆sgn◆RB◆(xy) = \mathrm◆LB◆sgn◆RB◆(x)\mathrm◆LB◆sgn◆RB◆(y)$. It is surjective. Its kernel is
$\{x \in \mathbb◆LB◆R◆RB◆^* : \mathrm◆LB◆sgn◆RB◆(x) = 1\} = \mathbb◆LB◆R◆RB◆^+$. By the first isomorphism theorem,
...
**Example 5.7 (Sign Homomorphism).** The sign map $\mathrm◆LB◆sgn◆RB◆ : S_n \to \{1, -1\}$ is a surjective
...
$$\{\mathrm◆LB◆subgroups\ of\ ◆RB◆ G \mathrm◆LB◆\ containing\ ◆RB◆ K\} \longleftrightarrow \{\mathrm◆LB◆subgroups\ of\ ◆RB◆ H\}$$
...
- $\Phi \circ \Psi = \mathrm◆LB◆id◆RB◆$: $\Phi(\Psi(V)) = \phi(\phi^◆LB◆-1◆RB◆(V)) = V$ (since $\phi$ is
  surjective).
- $\Psi \circ \Phi = \mathrm◆LB◆id◆RB◆$ on subgroups containing $K$: if $U \supseteq K$Then
...
Forms a group under composition, denoted $\mathrm◆LB◆Aut◆RB◆(G)$.

For each $g \in G$The **inner automorphism** $c_g : G \to G$ is defined by $c_g(x) = gxg^◆LB◆-1◆RB◆$. The
set of inner automorphisms $\mathrm◆LB◆Inn◆RB◆(G) = \{c_g : g \in G\}$ is a normal subgroup of
$\mathrm◆LB◆Aut◆RB◆(G)$.

**Proposition 5.7.** $\mathrm◆LB◆Inn◆RB◆(G) \cong G/Z(G)$.

_Proof._ Define $\psi : G \to \mathrm◆LB◆Aut◆RB◆(G)$ by $\psi(g) = c_g$. This is a homomorphism:
$\psi(gh) = c_◆LB◆gh◆RB◆$And
$c_◆LB◆gh◆RB◆(x) = ghx(gh)^◆LB◆-1◆RB◆ = g(hxh^◆LB◆-1◆RB◆)g^◆LB◆-1◆RB◆ = c_g(c_h(x)) = (c_g \circ c_h)(x)$. The image is
$\mathrm◆LB◆Inn◆RB◆(G)$. The kernel is
$\{g \in G : c_g = \mathrm◆LB◆id◆RB◆\} = \{g \in G : gxg^◆LB◆-1◆RB◆ = x
\ \mathrm◆LB◆for\ all\ x◆RB◆ \in G\} = Z(G)$. By
the first isomorphism theorem, $\mathrm◆LB◆Inn◆RB◆(G) \cong G/Z(G)$. $\blacksquare$

**Example.** $\mathrm◆LB◆Aut◆RB◆(S_3) \cong S_3$. Since $Z(S_3) = \{e\}$We have
$\mathrm◆LB◆Inn◆RB◆(S_3) \cong S_3$. Since $|\mathrm◆LB◆Aut◆RB◆(S_3)| \leq |S_3|! = 6$ (automorphisms permute the
three elements of order $2$), And $\mathrm◆LB◆Inn◆RB◆(S_3)$ already has $6$ elements, we get
$\mathrm◆LB◆Aut◆RB◆(S_3) = \mathrm◆LB◆Inn◆RB◆(S_3) \cong S_3$.

**Example.** $\mathrm◆LB◆Aut◆RB◆(\mathbb◆LB◆Z◆RB◆/n\mathbb◆LB◆Z◆RB◆) \cong (\mathbb◆LB◆Z◆RB◆/n\mathbb◆LB◆Z◆RB◆)^*$The group of
...
**Example.**
$\mathrm◆LB◆Aut◆RB◆(\mathbb◆LB◆Z◆RB◆/8\mathbb◆LB◆Z◆RB◆) \cong (\mathbb◆LB◆Z◆RB◆/8\mathbb◆LB◆Z◆RB◆)^* = \{1, 3, 5, 7\} \cong V_4$.
...
**Definition.** Let $H$ and $K$ be groups and let $\phi : K \to \mathrm◆LB◆Aut◆RB◆(H)$ be a homomorphism.
...
$\mathbb◆LB◆Z◆RB◆/2\mathbb◆LB◆Z◆RB◆ \to \mathrm◆LB◆Aut◆RB◆(\mathbb◆LB◆Z◆RB◆/3\mathbb◆LB◆Z◆RB◆) \cong \mathbb◆LB◆Z◆RB◆/2\mathbb◆LB◆Z◆RB◆$.
...
$\mathbb◆LB◆Z◆RB◆/3\mathbb◆LB◆Z◆RB◆ \to \mathrm◆LB◆Aut◆RB◆(\mathbb◆LB◆Z◆RB◆/7\mathbb◆LB◆Z◆RB◆) \cong \mathbb◆LB◆Z◆RB◆/6\mathbb◆LB◆Z◆RB◆$.

$\mathrm◆LB◆Aut◆RB◆(\mathbb◆LB◆Z◆RB◆/7\mathbb◆LB◆Z◆RB◆)$ has exactly one subgroup of order $3$ (generated by
...
The **orbit** of $x \in X$ is $\mathrm◆LB◆Orb◆RB◆(x) = \{g \cdot x : g \in G\}$.

The **stabilizer** of $x \in X$ is $\mathrm◆LB◆Stab◆RB◆(x) = \{g \in G : g \cdot x = x\}$.

**Proposition 6.1.** $\mathrm◆LB◆Stab◆RB◆(x)$ is a subgroup of $G$.

**Theorem 6.2 (Orbit-Stabilizer Theorem).** For any $x \in X$

$$|\mathrm◆LB◆Orb◆RB◆(x)| = [G : \mathrm◆LB◆Stab◆RB◆(x)] = \frac◆LB◆|G|◆RB◆◆LB◆|\mathrm◆LB◆Stab◆RB◆(x)|◆RB◆$$

_Proof._ Define $\phi : G \to \mathrm◆LB◆Orb◆RB◆(x)$ by $\phi(g) = g \cdot x$. Then $g$ and $h$ have the
same Image iff $g \cdot x = h \cdot x$ iff $h^◆LB◆-1◆RB◆g \cdot x = x$ iff $h^◆LB◆-1◆RB◆g \in \mathrm◆LB◆Stab◆RB◆(x)$
Iff $g \in h\,\mathrm◆LB◆Stab◆RB◆(x)$. So the fibers of $\phi$ are precisely the cosets of
$\mathrm◆LB◆Stab◆RB◆(x)$ And there are $[G : \mathrm◆LB◆Stab◆RB◆(x)]$ of them, each mapping to a distinct element
of $\mathrm◆LB◆Orb◆RB◆(x)$. $\blacksquare$
...
$$\frac◆LB◆1◆RB◆◆LB◆|G|◆RB◆ \sum_◆LB◆g \in G◆RB◆ |\mathrm◆LB◆Fix◆RB◆(g)|$$

Where $\mathrm◆LB◆Fix◆RB◆(g) = \{x \in X : g \cdot x = x\}$.

_Proof._ Count the set $S = \{(g, x) \in G \times X : g \cdot x = x\}$ in two ways. Grouping by $g$:
$|S| = \sum_◆LB◆g \in G◆RB◆ |\mathrm◆LB◆Fix◆RB◆(g)|$. Grouping by $x$: $|S| = \sum_◆LB◆x \in X◆RB◆ |\mathrm◆LB◆Stab◆RB◆(x)|$.
For $x$ in orbit $O$$|\mathrm◆LB◆Stab◆RB◆(x)| = |G|/|O|$. So
$\sum_◆LB◆x \in O◆RB◆ |\mathrm◆LB◆Stab◆RB◆(x)| = |O| \cdot |G|/|O| = |G|$. Summing over all orbits:
$|S| = |G| \cdot (\mathrm◆LB◆number\ of\ orbits◆RB◆)$. $\blacksquare$
...
non-central element $x$, $|\mathrm◆LB◆Orb◆RB◆(x)| = [G : C_G(x)]$ by the orbit-stabilizer theorem. Summing
...
rotated To any other), so $|\mathrm◆LB◆Orb◆RB◆(v)| = 8$. By orbit-stabilizer,
$|\mathrm◆LB◆Stab◆RB◆(v)| = 24/8 = 3$. Indeed, the stabilizer of a vertex consists of rotations about the
space diagonal through that vertex And its opposite: the identity, $120°$ rotation, and $240°$
rotation.

**Edges.** The cube has $12$ edges. The action is transitive, so $|\mathrm◆LB◆Orb◆RB◆(e)| = 12$ and
$|\mathrm◆LB◆Stab◆RB◆(e)| = 24/12 = 2$. The stabilizer of an edge is $\{\mathrm◆LB◆id◆RB◆, r\}$ where $r$ is the
$180°$ rotation about the axis through the midpoints of that edge and its opposite.

**Faces.** The cube has $6$ faces. The action is transitive, so $|\mathrm◆LB◆Orb◆RB◆(f)| = 6$ and
$|\mathrm◆LB◆Stab◆RB◆(f)| = 24/6 = 4$. The stabilizer of a face consists of rotations about the axis
...
multiplication. Since $|X|$ is not divisible by $p$Some orbit $\mathrm◆LB◆Orb◆RB◆(S)$ has size not
divisible by $p$. By the orbit-stabilizer theorem, $|\mathrm◆LB◆Stab◆RB◆(S)| = |G|/|\mathrm◆LB◆Orb◆RB◆(S)|$ is
divisible by $p^n$. For $s \in S$Left multiplication by $s$ is a bijection $S \to sS$And
$sS \subseteq S$ since $\mathrm◆LB◆Stab◆RB◆(S) \cdot S = S$. Since $|sS| = |S| = p^n$We get $sS = S$So
$s \in \mathrm◆LB◆Stab◆RB◆(S)$. Thus $S \subseteq \mathrm◆LB◆Stab◆RB◆(S)$Giving $p^n \leq |\mathrm◆LB◆Stab◆RB◆(S)|$.
Since $|\mathrm◆LB◆Stab◆RB◆(S)|$ divides $p^n m$ and is divisible by $p^n$We have $|\mathrm◆LB◆Stab◆RB◆(S)| = p^n$
And $\mathrm◆LB◆Stab◆RB◆(S)$ is a Sylow $p$-subgroup. $\blacksquare$
...
$$|\mathrm◆LB◆Fix◆RB◆(Q)| \equiv |G/P| \equiv m \not\equiv 0 \pmod◆LB◆p◆RB◆$$
...
_Proof._ Let $P$ be a Sylow $p$-subgroup. $P$ acts on the set $\mathrm◆LB◆Syl_p◆RB◆(G)$ of all Sylow
$p$-subgroups by conjugation. Write $\mathrm◆LB◆Syl_p◆RB◆(G) = \{P = P_1, P_2, \ldots, P_◆LB◆n_p◆RB◆\}$.
...
Thus $P$ is the **unique** fixed point. All other orbits have size $[P : \mathrm◆LB◆Stab_P◆RB◆(P_i)]$ A
power of $p$ greater than $1$. By the fixed-point congruence for $p$-group actions:
$n_p = 1 + \sum (\mathrm◆LB◆sizes\ of\ remaining\ orbits◆RB◆) \equiv 1 \pmod◆LB◆p◆RB◆$. **Step 2: $n_p$ divides
$m$.** The group $G$ acts transitively on $\mathrm◆LB◆Syl_p◆RB◆(G)$ by conjugation (by Sylow's second
theorem). Hence $n_p = |\mathrm◆LB◆Syl_p◆RB◆(G)| = [G : N_G(P)]$. Since $P \leq N_G(P)$, $|N_G(P)|$ is
...
The kernel is $\ker(\phi) = \{r : r \in I\ \mathrm◆LB◆and\ r◆RB◆ \in J\} = I \cap J$. By the ring
...
$(m) + (n) = (1) = \mathbb◆LB◆Z◆RB◆$. Also $(m) \cap (n) = (\mathrm◆LB◆lcm◆RB◆(m, n)) = (mn)$. $\blacksquare$
...
$$\mathrm◆LB◆Euclidean\ domain◆RB◆ \Rightarrow \mathrm◆LB◆PID◆RB◆ \Rightarrow \mathrm◆LB◆UFD◆RB◆$$
...
$|\mathrm◆LB◆Aut◆RB◆(\mathbb◆LB◆Q◆RB◆(\sqrt[4]◆LB◆2◆RB◆)/\mathbb◆LB◆Q◆RB◆)| = 2 < 4$. The extension is not Galois.
$\blacksquare$
...
Of $E/F$Denoted $\mathrm◆LB◆Gal◆RB◆(E/F)$.

**Example.** $\mathrm◆LB◆Gal◆RB◆(\mathbb◆LB◆C◆RB◆/\mathbb◆LB◆R◆RB◆) = \{id, \sigma\}$ where $\sigma(a + bi) = a - bi$.
This is isomorphic to $\mathbb◆LB◆Z◆RB◆/2\mathbb◆LB◆Z◆RB◆$.

### 13.2 The Fundamental Theorem of Galois Theory

A finite extension $E/F$ is **Galois** if $|\mathrm◆LB◆Gal◆RB◆(E/F)| = [E : F]$Or equivalently, If $E$ is
...
1. There is an inclusion-reversing bijection between intermediate fields $F \subseteq K \subseteq E$
   and subgroups $H \subseteq \mathrm◆LB◆Gal◆RB◆(E/F)$Given by:

- $K \mapsto \mathrm◆LB◆Gal◆RB◆(E/K)$.
- $H \mapsto E^H = \{x \in E : \sigma(x) = x\ \mathrm◆LB◆for\ all\ ◆RB◆\sigma \in H\}$.

2. $[E : K] = |\mathrm◆LB◆Gal◆RB◆(E/K)|$ and $[K : F] = [\mathrm◆LB◆Gal◆RB◆(E/F) : \mathrm◆LB◆Gal◆RB◆(E/K)]$.

3. $K/F$ is Galois if and only if $\mathrm◆LB◆Gal◆RB◆(E/K) \trianglelefteq \mathrm◆LB◆Gal◆RB◆(E/F)$In which case
   $\mathrm◆LB◆Gal◆RB◆(K/F) \cong \mathrm◆LB◆Gal◆RB◆(E/F) / \mathrm◆LB◆Gal◆RB◆(E/K)$.
...
The Galois group $\mathrm◆LB◆Gal◆RB◆(E/\mathbb◆LB◆Q◆RB◆)$ acts as permutations of the three roots, so
$\mathrm◆LB◆Gal◆RB◆(E/\mathbb◆LB◆Q◆RB◆) \cong S_3$.
...
**Proposition 13.4.** Let $G = \mathrm◆LB◆Gal◆RB◆(f) \leq S_n$. Then $G \leq A_n$ (i.e., $G$ is contained
...
For any $\sigma \in G$, $\sigma(\delta) = \mathrm◆LB◆sgn◆RB◆(\sigma) \cdot \delta$. If $\sigma \in A_n$
...
$\mathrm◆LB◆Gal◆RB◆(x^3 - 3x + 1) \leq A_3 \cong \mathbb◆LB◆Z◆RB◆/3\mathbb◆LB◆Z◆RB◆$. Since the polynomial is
irreducible, The Galois group is transitive, so
$\mathrm◆LB◆Gal◆RB◆(x^3 - 3x + 1) = A_3 \cong \mathbb◆LB◆Z◆RB◆/3\mathbb◆LB◆Z◆RB◆$.
...
Therefore $\mathrm◆LB◆Gal◆RB◆(E/\mathbb◆LB◆Q◆RB◆) \cong D_4$ (dihedral group of order $8$). $\blacksquare$
...
$\ker(\phi) \subseteq N$So $|\ker(\phi)|$ divides $8$. Since $\mathrm◆LB◆im◆RB◆(\phi) \subseteq S_3$ has
...
With $\tau\sigma\tau^◆LB◆-1◆RB◆ = \sigma^◆LB◆-1◆RB◆$We get $\mathrm◆LB◆Gal◆RB◆(E/\mathbb◆LB◆Q◆RB◆) \cong D_8$ (the dihedral
...
Automorphisms on $H$. The group of automorphisms of $H$, $\mathrm◆LB◆Aut◆RB◆(H)$Has order dividing $|H|!$But
...
Then $x^a = x^◆LB◆1 - bn◆RB◆ = x \cdot (x^n)^◆LB◆-b◆RB◆ = x \cdot x^◆LB◆-bn◆RB◆$... Actually, the general
...
$G/\ker(\phi) \cong \mathrm◆LB◆im◆RB◆(\phi)$ Not $G/\ker(\phi) \cong H$ (unless $\phi$ is surjective). :::
...
of $\mathrm◆LB◆Gal◆RB◆(E/F)$ is not a bijection, and indices may not match. Always verify the Galois
...
$\mathrm◆LB◆im◆RB◆(\phi) = 3\mathbb◆LB◆Z◆RB◆ = \{3k : k \in \mathbb◆LB◆Z◆RB◆\}$.
...
_Solution._ Let $x \in X$. Since $G$ acts transitively, $|\mathrm◆LB◆Orb◆RB◆(x)| = |X| = p$. By the
orbit-stabilizer theorem, $[G : \mathrm◆LB◆Stab◆RB◆(x)] = p$So $\mathrm◆LB◆Stab◆RB◆(x)$ has index $p$ in $G$.
Since $\mathrm◆LB◆Stab◆RB◆(x)$ is a subgroup (Proposition 6.1), we are done. $\blacksquare$
...
$\sqrt◆LB◆3◆RB◆$: $\mathrm◆LB◆id◆RB◆$: $\sqrt◆LB◆2◆RB◆ \mapsto \sqrt◆LB◆2◆RB◆$$\sqrt◆LB◆3◆RB◆ \mapsto \sqrt◆LB◆3◆RB◆$ $\sigma$:
...
$2$$\mathrm◆LB◆Gal◆RB◆(\mathbb◆LB◆Q◆RB◆(\sqrt◆LB◆2◆RB◆, \sqrt◆LB◆3◆RB◆)/\mathbb◆LB◆Q◆RB◆) \cong V_4$. $\blacksquare$
...
number of orbits equals $\frac◆LB◆1◆RB◆◆LB◆|G|◆RB◆ \sum_◆LB◆g \in G◆RB◆ |\mathrm◆LB◆Fix◆RB◆(g)|$.
...
**Grouping by $g$:**
$|S| = \sum_◆LB◆g \in G◆RB◆ |\{x \in X : g \cdot x = x\}| = \sum_◆LB◆g \in G◆RB◆ |\mathrm◆LB◆Fix◆RB◆(g)|$.

**Grouping by $x$:** $|S| = \sum_◆LB◆x \in X◆RB◆ |\mathrm◆LB◆Stab◆RB◆(x)|$.

For each orbit $\mathcal◆LB◆O◆RB◆$Every $x \in \mathcal◆LB◆O◆RB◆$ has $|\mathrm◆LB◆Stab◆RB◆(x)| = |G|/|\mathcal◆LB◆O◆RB◆|$
(by orbit-stabilizer). So
$\sum_◆LB◆x \in \mathcal◆LB◆O◆RB◆◆RB◆ |\mathrm◆LB◆Stab◆RB◆(x)| = |\mathcal◆LB◆O◆RB◆| \cdot |G|/|\mathcal◆LB◆O◆RB◆| = |G|$.

Summing over all orbits: $|S| = |G| \cdot (\mathrm◆LB◆number\ of\ orbits◆RB◆)$.

Combining: $\sum_◆LB◆g \in G◆RB◆ |\mathrm◆LB◆Fix◆RB◆(g)| = |G| \cdot (\mathrm◆LB◆number\ of\ orbits◆RB◆)$.
$\blacksquare$
...
| $\mathrm◆LB◆Inn◆RB◆(G) \cong G/Z(G)$                           | Proposition 5.7    |
...
- **Confusing groups, rings, and fields.** Group: one operation, inverses, identity. Ring: two
  ...
  multiplicative inverse. **Fix:** $\mathbb◆LB◆Z◆RB◆$ is a ring but not a field ($2$ has no multiplicative
  inverse). $\mathbb◆LB◆Q◆RB◆$, $\mathbb◆LB◆R◆RB◆$, $\mathbb◆LB◆C◆RB◆$ are fields.
...
- **Confusing normal subgroups and general subgroups.** Normal subgroups satisfy $gHg^◆LB◆-1◆RB◆ = H$ for
...
**Problem.** Show that $(\mathbb◆LB◆Z◆RB◆_n, +)$ is a group for any $n \in \mathbb◆LB◆N◆RB◆$.

**Solution.** (1) Closure: $[a] + [b] = [a + b \bmod n] \in \mathbb◆LB◆Z◆RB◆_n$. (2) Associativity:
inherited from $\mathbb◆LB◆Z◆RB◆$. (3) Identity: $[0]$. (4) Inverse: $[a]^◆LB◆-1◆RB◆ = [-a] = [n - a]$.
Therefore $(\mathbb◆LB◆Z◆RB◆_n, +)$ is a group. $\blacksquare$
...
- Cyclic group: generated by a single element $\langle g \rangle$; $\mathbb◆LB◆Z◆RB◆_n$ is cyclic.
