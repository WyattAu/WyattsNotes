...
**Definition 1 (Rate Law):** For many reactions, the rate is proportional to the concentrations
of reactants raised to powers:
...
**Example 1:** Radioactive decay of ${}^{14}\text{C}$ has $t_{1/2} = 5730$ years. What fraction remains
after 10000 years?
...
**Example 2:** A reaction has $k_1 = 3.46 \times 10^{-5}$ s$^{-1}$ at 298 K and $k_2 = 1.35 \times 10^{-3}$
s$^{-1}$ at 350 K. Find $E_a$.
...
**Definition 2 (Steric Factor):** Not every collision leads to reaction. The steric factor $P$ accounts
for orientation requirements:
...
$\Delta^{\ddagger} H^\circ$, and $\Delta^{\ddagger} S^\circ$ are the standard Gibbs energy, enthalpy,
...
**Definition 4 (Mechanism):** A reaction mechanism is a sequence of elementary steps that accounts for
the overall stoichiometry and the observed rate law.
...
where $V_{\max} = k_2[E]_0$ is the maximum velocity and $K_M = (k_{-1} + k_2)/k_1$ is the
Michaelis constant.
...
- **Heterogeneous catalysis:** Catalyst in a different phase (usually solid catalyst, gaseous/liquid
  reactants). Involves adsorption, surface reaction, and desorption.
...
1. **Confusing molecularity with reaction order.** Molecularity applies only to elementary steps; the
   overall reaction order is determined experimentally. **Fix:** Never assign orders from the balanced
   equation unless the reaction is known to be elementary.
2. **Using integrated rate laws for non-elementary reactions.** The integrated forms assume a single step
   of that order. **Fix:** First determine the rate law experimentally, then check which integrated form
   is consistent.
...
4. **Wrong activation energy units in the Arrhenius equation.** $E_a$ must be in J/mol (not kJ/mol) when
   using $R = 8.314$ J/(mol·K). **Fix:** Always convert to consistent units before substituting.
5. **Confusing $K_M$ with $K_d$ (dissociation constant).** $K_M = (k_{-1} + k_2)/k_1$, not $k_{-1}/k_1$.
   **Fix:** $K_M = K_d$ only when $k_2 \ll k_{-1}$.
...
7. **Forgetting that the Eyring equation uses $\Delta^{\ddagger} H^\circ$, not $E_a$.**
   $E_a = \Delta^{\ddagger} H^\circ + RT$. **Fix:** For reactions in solution, $E_a \approx \Delta^{\ddagger} H^\circ$,
   but in the gas phase the $RT$ term matters at high temperatures.
