...
**Machine epsilon** ($\epsilon$) is the smallest number such that $1 + \epsilon > 1$ in floating-point arithmetic. For IEEE 754 single precision, $\epsilon = 2^{-23} \approx 1.19 \times 10^{-7}$.
...
:::warning Pitfall
Never use `==` to compare floating-point numbers. Instead, check if $|a - b| < \epsilon$ for some tolerance.
:::
...
:::info Board-specific: CIE (9618)
CIE uses a simplified floating-point format:
