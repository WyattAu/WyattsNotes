...
**Invariant:** At the start of each loop iteration, if $x$ exists in $A$, then $x \in A[\text{low}..\text{high}]$.

**Base case.** Initially, `low = 0` and `high = n-1`, so $A[\text{low}..\text{high}] = A[0..n-1] = A$. If $x \in A$, the invariant holds.

**Maintenance.** Three cases:

1. $A[\text{mid}] = x$: Return mid. Correct. ✓
2. $A[\text{mid}] < x$: Since $A$ is sorted, $A[0..\text{mid}] \leq A[\text{mid}] < x$, so $x \notin A[0..\text{mid}]$. Setting `low = mid + 1` restricts the search to $A[\text{mid}+1..\text{high}]$. If $x$ was in the old range, it is in the new range.
3. $A[\text{mid}] > x$: Since $A$ is sorted, $A[\text{mid}..n-1] \geq A[\text{mid}] > x$, so $x \notin A[\text{mid}..n-1]$. Setting `high = mid - 1` restricts the search to $A[\text{low}..\text{mid}-1]$. If $x$ was in the old range, it is in the new range.

**Termination.** The loop terminates when `low > high`, meaning $A[\text{low}..\text{high}]$ is empty. By the invariant, $x \notin A$. Return $-1$. ✓
...
:::tip Exam tip
For exam questions, always state the precondition (sorted array) for binary search and trace through the algorithm step by step. Show the low, high, mid values at each iteration.
:::
