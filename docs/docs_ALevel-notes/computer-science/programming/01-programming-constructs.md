...
:::warning Pitfall
In Python, `int(3.9)` truncates toward zero (gives 3), not rounds. Use `round(3.9)` for rounding.
:::
...
- _Maintenance:_ `total` increases by $i$, then $i$ increases by 1. After: `total = 1 + ... + i`, and next $i' = i + 1$, so invariant holds.
...
**Proof.** Define a **variant function** $V(n) = n$. Each recursive call decreases $V$ by 1: $V(n-1) = n - 1 < n = V(n)$. $V$ is a non-negative integer that strictly decreases. By the well-ordering principle, $V$ must eventually reach a base case ($V = 0$ or $V = 1$). Therefore, the recursion terminates. $\square$
...
**Complexity:** $T(n) = T(n-1) + T(n-2) + O(1)$. This gives $T(n) = \Theta(\phi^n)$ where $\phi = \frac{1+\sqrt{5}}{2} \approx 1.618$ (the golden ratio).

**Proof sketch.** The recurrence has characteristic equation $r^2 = r + 1$, giving roots $\phi$ and $\psi = \frac{1-\sqrt{5}}{2}$. The solution is $T(n) = A\phi^n + B\psi^n$. Since $|\psi| < 1$, $T(n) = \Theta(\phi^n)$. $\square$

:::warning Pitfall
Naive recursive Fibonacci has exponential time complexity. Use memoisation or iteration for $O(n)$ time:
...
Base case ($d = 1$): $n < 10$, returns $n$. Sum of digits = $n$. ✓
...
**Termination.** Variant: $V(a, b) = b$. Each call: $V(a, b) = b > a \bmod b = V(b, a \bmod b)$ (for $b > 0$). Since $V$ is a non-negative integer that strictly decreases, the function reaches $b = 0$. ✓
