...
- **One-tailed:** $H_1: p > p_0$ (right-tailed) or $H_1: p < p_0$ (left-tailed).
- **Two-tailed:** $H_1: p \neq p_0$.
...
:::warning
Choosing a one-tailed test after seeing the data (because the results happen to go in one direction) is a form of $p$-hacking and is statistically invalid. The tail direction must be decided before the experiment.
:::
...
**Definition.** The **significance level** $\alpha$ is the maximum probability of incorrectly rejecting $H_0$ when it is true. Common values: 1%, 5%, 10%.
...
The **critical region** (rejection region) is the set of values of the test statistic that lead to rejection of $H_0$.

### 2.4 Actual significance level

For discrete distributions, the actual significance level may differ from the nominal level $\alpha$ because we cannot achieve exactly $\alpha$.
...
Decreasing $\alpha$ (making the test stricter) generally **increases** $\beta$ (more false negatives). There is always a trade-off between Type I and Type II errors.
...
Alternatively:
1–3. Same as above. 4. Calculate the **$p$-value**: the probability of obtaining a result at least as extreme as the observed value, assuming $H_0$ is true. 5. If $p$-value $< \alpha$, reject $H_0$. Otherwise, do not reject $H_0$. 6. Conclude in context.
...
$H_0: p = 0.5$, $H_1: p > 0.5$. One-tailed, $\alpha = 0.05$.

Under $H_0$: $X \sim B(20, 0.5)$.

Find $c$ such that $P(X \geq c) \leq 0.05$.

$P(X \geq 14) = 1 - P(X \leq 13) \approx 0.0577 > 0.05$.
$P(X \geq 15) = 1 - P(X \leq 14) \approx 0.0207 < 0.05$.
...
**Example.** A machine fills bags with mean weight 500g. A sample of 30 bags gives $\bar{x} = 497$g. Test at the 5% level whether the mean weight has decreased, given $\sigma = 6$g.

$H_0: \mu = 500$, $H_1: \mu < 500$. $\alpha = 0.05$.

Under $H_0$: $\bar{X} \sim N(500, 6^2/30) = N(500, 1.2)$.

$z = \dfrac{497 - 500}{\sqrt{1.2}} = \dfrac{-3}{1.0954} = -2.739$.

Critical value: $P(Z < -1.645) = 0.05$.

Since $-2.739 < -1.645$, we reject $H_0$.
...
:::warning
"Failing to reject $H_0$" is **not** the same as "proving $H_0$ is true." It means the data does not provide sufficient evidence against $H_0$. The test may lack power (sample too small, effect too weak).
:::
...
Two-tailed: critical values $\pm 1.96$. $|2.078| > 1.96$, so **reject** $H_0$.
...
Critical value: $-1.645$. Since $-2.358 < -1.645$, **reject** $H_0$.
...
$P(X \geq 5) = 1 - P(X \leq 4) = 1 - 0.8497 = 0.1503 > 0.05$.
...
Critical value at 1%: $-2.326$. Since $-2.652 < -2.326$, **reject** $H_0$.
...
Lower: $P(X \leq 5) \approx 0.0207 \leq 0.05$, $P(X \leq 6) \approx 0.0577 > 0.05$. So $c_L = 5$.
Upper: $P(X \geq 15) \approx 0.0207 \leq 0.05$, $P(X \geq 14) \approx 0.0577 > 0.05$. So $c_U = 15$.
...
Two-tailed critical values: $\pm 1.96$. $|-1.667| < 1.96$, so **do not reject** $H_0$.
...
$P(X \geq 19) = 1 - P(X \leq 18) \approx 1 - 0.9264 = 0.0736 > 0.05$.
$P(X \geq 20) = 1 - P(X \leq 19) \approx 1 - 0.9773 = 0.0227 < 0.05$.

Critical region: $X \geq 20$. Since $X = 18 < 20$, **do not reject** $H_0$.
...
$P(\bar{X} < 51.645 \mid \mu = 52) = P\!\left(Z < \dfrac{51.645 - 52}{1}\right) = P(Z < -0.355) \approx 0.3613$.
