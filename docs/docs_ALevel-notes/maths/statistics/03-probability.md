...
($\Rightarrow$) If $P(A \cap B) = P(A)P(B)$, then $P(A|B) = \dfrac{P(A \cap B)}{P(B)} = \dfrac{P(A)P(B)}{P(B)} = P(A)$.

($\Leftarrow$) If $P(A|B) = P(A)$, then $\dfrac{P(A \cap B)}{P(B)} = P(A)$, so $P(A \cap B) = P(A)P(B)$. $\blacksquare$
...
:::warning
"Independent" and "mutually exclusive" are **different** concepts. If $A$ and $B$ are mutually exclusive and both have positive probability, they are **not** independent (since $P(A \cap B) = 0 \neq P(A)P(B)$).
:::
...
When all outcomes are equally likely: $P(A) = \dfrac{|A|}{|\Omega|} = \dfrac{\text{number of favourable outcomes}}{\text{total number of outcomes}}$.
...
By Bayes' theorem:
$P(D|T^+) = \dfrac{P(T^+|D)P(D)}{P(T^+)} = \dfrac{0.0099}{0.0198} = 0.5$.
