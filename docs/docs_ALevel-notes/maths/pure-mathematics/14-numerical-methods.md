...
**Theorem.** If $g$ is continuously differentiable near a fixed point $\alpha$ and $|g'(\alpha)| < 1$, then the iteration $x_{n+1} = g(x_n)$ converges to $\alpha$ for all starting values sufficiently close to $\alpha$.

If $|g'(\alpha)| > 1$, the iteration diverges.
...
If $|g'(\alpha)| < 1$, then $|x_{n+1} - \alpha| < |x_n - \alpha|$: the error shrinks, so the iteration converges. If $|g'(\alpha)| > 1$, the error grows and the iteration diverges. $\blacksquare$
...
- $g(x) = 1 - x^3$: $g'(x) = -3x^2$. Near the root $\alpha \approx 0.68$: $|g'(\alpha)| \approx 1.39 > 1$. **Diverges.**
- $g(x) = \sqrt[3]{1-x}$: $g'(x) = \dfrac{-1}{3(1-x)^{2/3}}$. Near $\alpha$: $|g'(\alpha)| \approx 0.72 < 1$. **Converges.**

:::tip
In exams, if a question asks you to show that a particular rearrangement converges, compute $g'(x)$ at the root and show $|g'(\alpha)| < 1$. If asked why a rearrangement fails, show $|g'(\alpha)| > 1$.
:::
...
Taking absolute values: $|x_{n+1} - \alpha| = \dfrac{|f''(\xi)|}{2|f'(x_n)|}|x_n - \alpha|^2 \leq C|x_n - \alpha|^2$. $\blacksquare$
...
:::warning
Always check that Newton-Raphson is converging. If the iterates oscillate or diverge, try a different starting point.
:::
...
| Fixed-point    | Linear      | $\|g'\| < 1$       | Moderate     |
...
$f(\ln 3) = 3 - 3\ln 3 \approx 3 - 3.296 = -0.296 < 0$.

Since $f''(x) = e^x > 0$, this is a global minimum. The minimum value is negative, and $f(x) \to \infty$ as $x \to \pm\infty$, so $f(x) = 0$ has exactly two roots.
...
$|g'(\alpha)| = \dfrac{1}{3}(3-1.21)^{-2/3} = \dfrac{1}{3}(1.79)^{-2/3} \approx \dfrac{1}{3 \times 1.489} \approx 0.224 < 1$.
