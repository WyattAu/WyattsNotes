...
:::warning
A vector equation of a line is not unique. Different points on the line can be used as $\mathbf{a}$, and the direction vector $\mathbf{b}$ can be any non-zero scalar multiple of the direction. Always check your answer gives a point and direction consistent with the question.
:::
