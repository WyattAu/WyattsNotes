...
where $\mathbf{r}_i$ is the position vector of the point of application of $\mathbf{F}_i$ relative to $O$.

This means the clockwise and anticlockwise moments balance: $\sum M_{\text{clockwise}} = \sum M_{\text{anticlockwise}}$. $\blacksquare$
...
For a body made of several parts with masses $m_1, m_2, \ldots$ at positions $(x_1, y_1), (x_2, y_2), \ldots$:

$$\bar{x} = \frac{\sum m_i x_i}{\sum m_i}, \qquad \bar{y} = \frac{\sum m_i y_i}{\sum m_i}$$

**Derivation.** Taking moments about the $y$-axis for the total system and the equivalent point mass:
...
For the reaction at $A \geq 0$: $3W(l-x) \leq W \cdot l \implies 3l - 3x \leq l \implies x \geq \dfrac{2l}{3}$.
