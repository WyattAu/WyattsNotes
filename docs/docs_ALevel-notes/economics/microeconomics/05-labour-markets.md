...
$$\bar{w}_m - \bar{w}_f = \underbrace{\bar{X}_m \hat{\beta}_m - \bar{X}_f \hat{\beta}_m}_{\text{Explained (characteristics)}} + \underbrace{\bar{X}_f \hat{\beta}_m - \bar{X}_f \hat{\beta}_f}_{\text{Unexplained (discrimination)}}$$
