...
:::tip
When integrating $e^{ax}\cos bx$ or $e^{ax}\sin bx$, apply integration by parts twice. The
...
:::warning
The boundary term $[-\sin^{n-1}x\cos x]_0^{\pi/2}$ vanishes only because the limits are
...
:::tip
The standard strategy is: (1) complete the square in the denominator, (2) split the numerator
...
:::warning
Distinguish carefully between $\dfrac{1}{a^2+x^2}$ (gives $\arctan$),
