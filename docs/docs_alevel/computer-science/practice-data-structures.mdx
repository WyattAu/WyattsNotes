...
<PracticeProblem
  questions={[
    ...
    },
    {
      ...
      correct: 1,
      explanation:
        "In a singly linked list, deleting a node requires traversing from the head to find the predecessor (O(n) worst case) to update its next pointer. In a doubly linked list, each node stores a pointer to its predecessor, so deletion given a reference to the node is O(1) -- update the predecessor's next pointer and the successor's previous pointer. The trade-off is extra memory: each node stores two pointers instead of one.",
    },
    {
      question:
      ...
      correct: 1,
      explanation:
        "For a standard singly linked list, finding the middle requires traversing from the head, taking O(n) time (even with Floyd's tortoise and hare algorithm, it still visits O(n) nodes). For a circular singly linked list, even with a tail pointer, you cannot directly access the middle -- you must still traverse n/2 nodes from any starting point. O(n/2) simplifies to O(n). Circular linked lists provide O(1) access to head (via tail.next) and enable round-robin scheduling, but do not improve middle-element access.",
    },
  ]}
/>
  ...
  questions={[
    {
      ...
      correct: 1,
      explanation:
        'A stack follows LIFO -- the last element pushed is the first one popped. Think of a stack of plates: the top plate is always removed first. Core operations are push (add to top) and pop (remove from top), both O(1). Peek examines the top element without removing it. Stacks are used in recursion (call stack), undo/redo operations, bracket matching, and postfix expression evaluation.',
    },
    {
      question:
        'A circular queue is implemented using an array of size 5. The front pointer is at index 2 and the rear pointer is at index 0. After enqueuing elements A, B, C (in that order), what are the new front and rear values?',
      options: [
        'A) Front = 2, Rear = 3',
        'B) Front = 2, Rear = 0',
        'C) Front = 2, Rear = 2',
      ...
      correct: 0,
      explanation:
        'In a circular queue, the rear pointer advances using rear = (rear + 1) MOD capacity. Starting with rear = 0: enqueue A -> rear = (0+1) MOD 5 = 1. Enqueue B -> rear = (1+1) MOD 5 = 2. Enqueue C -> rear = (2+1) MOD 5 = 3. The front pointer remains at 2 (front only changes on dequeue). Result: front = 2, rear = 3. Circular queues efficiently reuse array space that would be wasted in a linear queue implementation.',
    },
    {
      question:
        'When evaluating the postfix expression "5 3 2 * + 4 -", what is the sequence of stack operations and the final result?',
      options: [
        'A) Push 5, push 3, push 2; pop 2 and 3, multiply = 6, push 6; pop 6 and 5, add = 11, push 11; push 4; pop 4 and 11, subtract = 7. Result: 7',
        'B) Push 5, push 3, multiply 5 and 3 = 15, push 2, add 15 and 2 = 17, push 4, subtract 17 and 4 = 13. Result: 13',
        'C) Push 5, push 3, push 2; pop 2 and 3, multiply = 6, push 6; pop 6 and 5, add = 11; pop 11, push 4, subtract = 7. Result: 7',
        'D) Push 5, push 3, push 2, multiply 3 and 2 = 6, add 6 and 5 = 11, subtract 11 and 4 = 7, push 7. Result: 7',
      ],
      correct: 0,
      explanation:
        'Postfix evaluation using a stack: (1) Push 5. (2) Push 3. (3) Push 2. (4) Read * -- pop 2 and 3, compute 3*2=6, push 6. Stack: [5, 6]. (5) Read + -- pop 6 and 5, compute 5+6=11, push 11. Stack: [11]. (6) Push 4. Stack: [11, 4]. (7) Read - -- pop 4 and 11, compute 11-4=7, push 7. Stack: [7]. Result: 7. This is equivalent to infix (5 + (3 * 2)) - 4 = 7.',
    },
  ]}
/>
...
<PracticeProblem
  questions={[
    {
      question:
        'Which statement correctly describes the ordering property of a binary search tree (BST)?',
      options: [
        'A) Every node has exactly two children',
        'B) For every node, all values in the left subtree are less than the node, and all values in the right subtree are greater',
        'C) The tree must be perfectly balanced to function correctly',
        'D) Values are stored only in the leaf nodes',
      ],
      correct: 1,
      explanation:
        'A BST maintains the ordering property: for any node, left subtree values < node value < right subtree values. This enables O(log n) average-case search, insertion, and deletion. Nodes can have 0, 1, or 2 children (not exactly two). BSTs are not required to be balanced, though balance affects performance -- a degenerate BST (inserted in sorted order) becomes a linked list with O(n) operations.',
    },
    {
      question:
        'A BST has the following values inserted in order: 50, 30, 70, 20, 40, 60, 80. What is the result of an in-order traversal?',
      options: [
      ...
      correct: 2,
      explanation:
        'In-order traversal visits left subtree, node, right subtree (L-N-R). The tree structure: root 50, left child 30 (with children 20 and 40), right child 70 (with children 60 and 80). In-order: 20, 30, 40, 50, 60, 70, 80. For any BST, in-order traversal always produces values in ascending sorted order. This property is used to verify BST correctness and to extract sorted data.',
    },
    {
      question:
        'An AVL tree has nodes inserted in order: 10, 20, 30. After inserting 30, the tree becomes unbalanced. What rotation is needed and what is the resulting structure?',
      options: [
        'A) Right rotation on node 10; resulting root is 20 with left child 10 and right child 30',
        'B) Left rotation on node 10; resulting root is 20 with left child 10 and right child 30',
        'C) Left rotation on node 10, then right rotation on node 30; resulting root is 20',
        'D) Left-right double rotation on node 10; resulting root is 30',
      ],
      correct: 1,
      explanation:
        'Inserting 10, 20, 30 creates a chain: 10 -> right: 20 -> right: 30. Node 10 has balance factor -2 (right-heavy), and its right child (20) has balance factor -1 (also right-heavy). This is a Right-Right (RR) case requiring a single left rotation on node 10. After the left rotation: 20 becomes the new root, 10 becomes left child of 20, 30 remains right child of 20. All balance factors become 0. In AVL trees, single rotations handle LL and RR cases; double rotations handle LR and RL cases.',
    },
  ]}
/>
...
<PracticeProblem
  questions={[
    {
      question:
        'For a directed graph with 4 vertices and 5 edges, how many cells in the adjacency matrix will contain a non-zero value?',
      options: ['A) 5', 'B) 10', 'C) 16', 'D) 8'],
      correct: 0,
      explanation:
        'In an adjacency matrix for a directed graph, each directed edge is represented by exactly one non-zero entry at position [source][destination]. With 5 edges, there are exactly 5 non-zero cells. The matrix itself is 4x4 = 16 cells, but only 5 contain values. For undirected graphs, each edge appears twice (at [i][j] and [j][i]), so 5 edges would produce 10 non-zero cells.',
    },
    {
      question:
        'In an unweighted graph, which algorithm guarantees finding the shortest path between two vertices and what data structure does it use?',
      options: [
        'A) DFS using a stack',
        'B) BFS using a queue',
      ...
      correct: 1,
      explanation:
        "BFS finds the shortest path in unweighted graphs by exploring all vertices at distance d before distance d+1, using a FIFO queue. It guarantees the shortest path because it explores in order of increasing distance from the source. DFS (using a stack) does NOT guarantee the shortest path -- it may find a deep path before a shallow one. Dijkstra's algorithm handles weighted graphs using a priority queue, and A* uses heuristics for faster pathfinding.",
    },
    {
      question:
