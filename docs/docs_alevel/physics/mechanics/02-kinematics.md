...
$$\boxed{v_T \text{ is reached when } F_{\text{drag}} = mg \implies a = 0}$$
...
$$\mathbf{v}_{\text{ground}} = (3.0\ \text{east}) + (5.0\ \text{north})$$
...
$\mathbf{v}_{BA} = \mathbf{v}_B - \mathbf{v}_A = (10\ \text{east}) - (10\ \text{north}) = (10, -10)$
m s$^{-1}$.
...
<b>If you get this wrong, revise:</b> [Free Fall](#3-free-fall) and
...
**Answer.** $v = \frac{ds}{dt}$ and $a = \frac{dv}{dt} = \text{const}$. Integrating:
