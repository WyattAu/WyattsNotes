...
description: "20 interactive flashcards for A-Level Further Maths further statistics covering PMF, E(X), Var(X), coding, continuous distributions, hypothesis testing, t-distribution, chi-squared, and power of tests."
tags:
  - Further Maths
  - A-Level
  - Flashcards
  ...
  cards={[
    {
      ...
      front: 'What is the difference between a probability mass function (PMF) and a probability density function (PDF)?',
    ...
    },
    {
      ...
      front: 'State Var(X) = E(X²) - [E(X)]² and explain why this form is preferred over Var(X) = E[(X - μ)²].',
    ...
    },
    {
      ...
      back: 'E(Y) = aE(X) + b. Var(Y) = a²Var(X). The mean scales by a and shifts by b. The variance scales by a² only; adding a constant b does not change the spread. Standard deviation: σ_Y = |a|σ_X. These rules are fundamental for working with coded data in exam questions.',
      tags: ['coding'],
      difficulty: 'medium',
    },
    {
    ...
    },
    {
      ...
      back: 'M_X(t) = E(e^(tX)) = Σ e^(tx)P(X=x) (discrete) or ∫ e^(tx)f(x)dx (continuous). Key uses: (1) E(X) = M\'_X(0), (2) E(X²) = M\'\'_X(0), (3) Var(X) = M\'\'_X(0) - [M\'_X(0)]², (4) If X and Y are independent: M_{X+Y}(t) = M_X(t) × M_Y(t). The MGF uniquely determines the distribution.',
      tags: ['mgf'],
      difficulty: 'hard',
    },
    {
    ...
    },
    {
      ...
      front: 'Explain how to determine the degrees of freedom for a chi-squared goodness of fit test.',
    ...
    },
    {
      ...
      front: 'Compare the t-distribution with the standard normal distribution. When do you use each?',
    ...
    },
    {
      ...
      front: 'Describe the chi-squared test for association in a contingency table and state Yates\' correction.',
