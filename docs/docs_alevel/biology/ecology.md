---
title: Ecology
description: "A-Level Biology notes on Ecology: 1. Ecosystems; 1.1 Definitions; 1.2 Trophic Levels; 1.3 Ecological Productivity with worked examples and exam-style questions."
...
The precise figure varies ( 80%--90% is lost, 10%--20% is transferred). The key point is
...
5. **Climax community**: the final, stable community in equilibrium with the climate. In the UK,
 this is oak or ash woodland. The climax community is self-sustaining and persists until
 a major disturbance.
...
Biomass at the next level ( 10--20%), not the fraction of total solar radiation captured by
Producers ( less than 3%).
:::
...
The predator population cycle lags behind the prey cycle because there is a time delay between the change in prey availability and the predator population response (predators need time to reproduce).
...
1. **Pioneer species**: lichens and algae colonise bare rock. Lichens secrete acids that weather the rock, beginning soil formation.
...
6. **Climax community** (e.g., oak-hickory forest in temperate regions) is reached. This is a stable, self-sustaining community that does not change significantly unless disrupted.
...
Where $S$ = number of species, $A$ = area, $c$ = constant (depends on the type of organism and habitat), $z$ = slope ( $0.2$--$0.35$ for islands).
...
1. **Embryo dunes**: pioneer plants such as **sea couch grass** and **marram grass** colonise the bare sand. Their long roots stabilise the sand and trap more sand, allowing the dune to grow.
2. **Fore dunes**: as sand accumulates, conditions become less harsh (more moisture, less wind). Plants such as **sea spurge** and **sea holly** colonise.
3. **Yellow dunes**: **marram grass** dominates. It has rolled leaves (reduces water loss), deep roots (anchors in sand), and can tolerate burial by sand.
4. **Grey dunes**: as organic matter accumulates from dead plant material, the soil becomes more fertile and retains more water. **Fixed dune** communities develop with species such as **restharrow**, **bird's-foot trefoil**, and **vetches**.
5. **Dune slack**: behind the dunes, a hollow may fill with fresh water (rainwater above the saline groundwater), forming a dune slack with wetland species (willow, rushes, orchids).
6. **Climax community**: eventually, shrubs and trees (oak, birch, pine) colonise, forming a woodland climax community.
...
1. **Nutrient input**: fertilisers (nitrates and phosphates) are leached from agricultural land or discharged in sewage into rivers and lakes.
...
1. **Range shifts**: species migrate towards the poles or to higher altitudes to track their optimal temperature. Species at the poles or mountain tops have nowhere to go.
...
**Competitive exclusion principle:** two species cannot coexist in the same niche (one will outcompete the other). If two species appear to occupy the same niche, closer inspection reveals niche differentiation (resource partitioning).
...
3. Multiply by the energy content per unit dry mass (approximately $18\ \mathrm{kJ\ g^{-1}$ for plant material).
...
:::tip
Tip
Ready to test your understanding of **Ecology**? The [diagnostic test](/docs/alevel/biology/diagnostics/diag-ecology) contains the hardest questions within the A-Level specification for this topic, each with a full worked solution.
