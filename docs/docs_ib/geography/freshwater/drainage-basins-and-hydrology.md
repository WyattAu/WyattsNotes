...
a common outlet, typically where a river enters a lake, sea, or ocean. The boundary of a drainage
...
period (typically the hydrological year), $\Delta S$ approaches zero, yielding:
...
A storm hydrograph (flood hydrograph) plots river discharge (typically in m$^3$/s on the y-axis)
against time (typically hours on the x-axis) for a specific rainfall event. The key features are:
...
(typically at 0.6 of the depth from the surface, or as the average of measurements at 0.2 and 0.8 of
