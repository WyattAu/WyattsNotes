...
slug: flashcards-waves
description:
  "Interactive flashcards for IB Physics waves covering transverse and longitudinal waves, the wave
  equation, Snell's law, interference, standing waves, the Doppler effect, and polarization."
...
const flashcardData = [
  {
    ...
    back: 'Transverse: particles oscillate perpendicular to direction of wave propagation. Examples: light, water surface waves, waves on a string, all EM waves.\nLongitudinal: particles oscillate parallel to direction of wave propagation. Examples: sound waves in air, compression waves in a slinky, P-waves (seismic).\nBoth transfer energy without transferring matter.',
  ...
  },
  {
    ...
    back: 'Radio → Microwave → Infrared → Visible (red to violet) → Ultraviolet → X-ray → Gamma ray.\nVisible spectrum: red (~700 nm) → orange → yellow → green → blue → indigo → violet (~400 nm).\nAll EM waves travel at c = 3.00 × 10⁸ m s⁻¹ in vacuum. All are transverse. All can travel through vacuum.',
  ...
  },
  {
    ...
    back: "Snell's law: n₁ sin θ₁ = n₂ sin θ₂. n = refractive index, θ = angle from normal.\nRefraction is the bending of light at a boundary between media of different refractive indices. Light bends towards the normal when entering a denser medium (higher n) and away when entering a less dense medium. The frequency stays constant; speed and wavelength change.",
  ...
  },
  {
    ...
    back: 'Two coherent sources needed (same frequency, constant phase relationship).\nConstructive interference: path difference = nλ (n = 0, 1, 2, ...). Waves arrive in phase, amplitudes add. Bright fringe (light) or loud sound.\nDestructive interference: path difference = (n + ½)λ (n = 0, 1, 2, ...). Waves arrive out of phase (half wavelength), amplitudes cancel. Dark fringe or silence.',
  ...
  },
  {
    ...
    back: 'Standing waves form when two identical waves travelling in opposite directions superpose. The wave appears stationary — it does not propagate energy.\nNodes: points of zero amplitude (always in phase cancelling). Antinodes: points of maximum amplitude (always in phase adding).\nAdjacent nodes are separated by λ/2. Adjacent node and antinode by λ/4. All points between nodes oscillate in phase.',
  ...
  },
  {
    ...
    back: 'For a source moving towards a stationary observer: f_observed = f_source × v/(v − v_source).\nFor a source moving away: f_observed = f_source × v/(v + v_source).\nv = wave speed in the medium, v_source = speed of source.\nApproaching source: observed frequency is HIGHER (wavelength compressed).\nReceding source: observed frequency is LOWER (wavelength stretched).\nApplications: radar guns, red shift in astronomy, medical ultrasound.',
  ...
  },
  {
    ...
    back: 'Fundamental (1st harmonic, n=1): one antinode at centre, two nodes at ends. λ₁ = 2L, f₁ = v/(2L).\n2nd harmonic (n=2): two antinodes, three nodes. λ₂ = L, f₂ = 2f₁.\nnth harmonic: λ_n = 2L/n, f_n = n × f₁ = nv/(2L).\nThe frequency of each harmonic is an integer multiple of the fundamental. Only odd and even harmonics are present (all integer multiples).',
  ...
  },
  {
    ...
    back: 'Open pipe (both ends open): antinodes at both ends. λ₁ = 2L, f₁ = v/(2L). All harmonics present: n = 1, 2, 3, ...\nClosed pipe (one end open, one closed): antinode at open end, node at closed end. λ₁ = 4L, f₁ = v/(4L). Only ODD harmonics present: n = 1, 3, 5, ...\nA closed pipe produces a fundamental frequency one octave lower than an open pipe of the same length.',
  ...
  },
  {
    ...
    back: "1) Diffraction: light spreads when passing through narrow slits (single-slit diffraction pattern).\n2) Interference: Young's double-slit experiment produces alternating bright and dark fringes.\n3) Polarization: light can be polarized, proving it is transverse.\nThese observations cannot be explained by particle models. The particle nature is shown by the photoelectric effect. Together: wave-particle duality.",
...
<FlashcardDeck
  deckId="ib-phys-waves"
  title="IB Physics: Waves"
  description="Wave properties, wave equation, electromagnetic spectrum, Snell's law, diffraction, interference, standing waves, Doppler effect, and polarization."
  cards={flashcardData}
/>;
