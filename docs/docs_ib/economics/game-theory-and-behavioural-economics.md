...
3. **The revelation principle**: any outcome achievable by any mechanism can also be achieved
   by a direct mechanism where participants simply report their types, provided truth-telling
...
Under certain conditions (risk-neutral bidders, independent private values, symmetric bidders),
...
disputes are resolved peacefully: the cost of war typically exceeds the value of the disputed
resource.
...
Where $\beta < 1$ captures present bias (typically $\beta \approx 0.7$) and $\delta$ is the
...
**Experimental finding:** contributions typically start at 40--60% of the socially optimal
