...
The SI unit is $\text{V m}^{-1}$ (equivalent to $\text{N C}^{-1}$). For a point charge $Q$, the
...
$v = 3.0 \times 10^7 \text{ m/s}$ horizontally. The plates are $5.0\text{ cm}$ long and
$2.0\text{ cm}$ apart, with a potential difference of $400\text{ V}$ across them.
...
$$
E = \frac{V}{d} = \frac{400}{0.020} = 2.0 \times 10^4 \text{ V/m}
$$

Time between the plates:

$$
t = \frac{L}{v} = \frac{0.050}{3.0 \times 10^7} = 1.67 \times 10^{-9}\text{ s}
$$

Vertical acceleration (note: the electron has charge $q = -1.6 \times 10^{-19}\text{ C}$; it is
deflected toward the positive plate):

$$
a = \frac{eE}{m_e} = \frac{(1.6 \times 10^{-19})(2.0 \times 10^4)}{9.11 \times 10^{-31}} = 3.51 \times 10^{15}\text{ m/s}^2
$$
...
The electron deflects $4.9\text{ mm}$ toward the positive plate. Since $y = 4.9\text{ mm}$ is less
than the half-separation of $10\text{ mm}$, the electron exits the plates without striking them.
...
The SI unit is the ampere ($\text{A}$), where $1\text{ A} = 1\text{ C s}^{-1}$. Conventional current
...
$e = 1.60 \times 10^{-19}\text{ C}$ and drifts with velocity $v_d$.

In time $\Delta t$, the volume swept by electrons crossing a given cross-section is:

$$
\text{Volume} = A \cdot v_d \cdot \Delta t
$$
...
:::info[Typical Values] For copper, $n \approx 8.5 \times 10^{28}\text{ m}^{-3}$. For a
$1\text{ mm}^2$ copper wire carrying $1\text{ A}$, the drift velocity is:

$$
v_d = \frac{I}{nAe} = \frac{1.0}{(8.5 \times 10^{28})(10^{-6})(1.6 \times 10^{-19})} \approx 7.4 \times 10^{-5}\text{ m/s}
$$
...
An aluminium wire of diameter $0.50\text{ mm}$ carries a current of $2.0\text{ A}$. For aluminium,
the free electron density is $n = 6.0 \times 10^{28}\text{ m}^{-3}$.

Cross-sectional area:

$$
A = \pi r^2 = \pi(0.25 \times 10^{-3})^2 = 1.96 \times 10^{-7}\text{ m}^2
$$

Drift velocity:

$$
v_d = \frac{I}{nAe} = \frac{2.0}{(6.0 \times 10^{28})(1.96 \times 10^{-7})(1.6 \times 10^{-19})} = 1.06 \times 10^{-3}\text{ m/s}
$$

The drift velocity is approximately $1.1\text{ mm/s}$.
...
SI unit: volt ($\text{V}$), where $1\text{ V} = 1\text{ J C}^{-1}$.
...
where $R$ is the resistance measured in ohms ($\Omega$), with $1\,\Omega = 1\text{ V A}^{-1}$.
...
The inverse quantity is conductivity: $\sigma = 1/\rho$, with SI unit $\text{S m}^{-1}$ (siemens per
metre).

:::info[Data Booklet Resistivity Values] Copper:
$\rho \approx 1.7 \times 10^{-8}\,\Omega\,\text{m}$. Nichrome (alloy):
$\rho \approx 1.1 \times 10^{-6}\,\Omega\,\text{m}$. Semiconductors: $\rho$ spans many orders of
magnitude.
:::
...
$\alpha \approx 3.9 \times 10^{-3}\text{ K}^{-1}$. The physical mechanism is that lattice vibrations
...
($\rho \lt 10^{-4}\,\Omega\,\text{m}$), semiconductors ($\rho$ between roughly $10^{-4}$ and
$10^{6}\,\Omega\,\text{m}$), and insulators ($\rho \gt 10^{6}\,\Omega\,\text{m}$) form a continuum.
...
threshold voltage is reached (approximately $0.7\text{ V}$ for silicon diodes), after which current
...
$$
V_1 = V_{\text{total}} \cdot \frac{R_1}{R_1 + R_2}
$$
...
Consider a circuit with two cells and three resistors. Cell 1 has emf $\varepsilon_1 = 12\text{ V}$
and cell 2 has emf $\varepsilon_2 = 6\text{ V}$ (opposing). The resistors are $R_1 = 4\,\Omega$,
...
The p.d. across each resistor: $V_1 = 2.0\text{ V}$, $V_2 = 3.0\text{ V}$, $V_3 = 1.0\text{ V}$.
These sum to $6\text{ V}$, which equals the net emf of $12 - 6 = 6\text{ V}$, confirming energy
conservation.
...
For two resistors $R_1$ (upper) and $R_2$ (lower) across supply $V_{\text{in}}$:
...
resistor increases, so $V_{\text{out}}$ measured across $R_2$ actually decreases). Careful analysis
...
$V_{\text{out}}$ across $R_2$ (fixed). As temperature rises, $R_1$ decreases, so more of
$V_{\text{in}}$ drops across $R_2$, and $V_{\text{out}}$ increases.
...
The "lost volts" is the p.d. across the internal resistance: $V_{\text{lost}} = Ir$.
...
The y-intercept equals $\varepsilon$. The gradient equals $-r$ (so $r = |\text{gradient}|$).

The x-intercept gives the short-circuit current $I_{\text{sc}} = \varepsilon / r$.
...
terminal p.d. is measured as $5.8\text{ V}$ and the current is $1.2\text{ A}$.
...
When the external resistor is changed to $9.0\,\Omega$, the current becomes $0.65\text{ A}$:

$$
V' = \varepsilon - I'r = (5.8 + 1.2r) - 0.65r = 5.8 + 0.55r
$$

But also $V' = I'R' = 0.65 \times 9.0 = 5.85\text{ V}$:
...
$$
\varepsilon = 5.8 + 1.2(0.091) = 5.91\text{ V}
$$
...
$$
\varepsilon_{\text{total}} = n\varepsilon, \quad r_{\text{total}} = nr
$$
...
$$
\varepsilon_{\text{total}} = \varepsilon, \quad r_{\text{total}} = \frac{r}{n}
$$
...
$$
\varepsilon_{\text{total}} = m\varepsilon, \quad r_{\text{total}} = \frac{mr}{n}
$$
...
$$
P_{\text{total}} = \varepsilon I
$$

The power delivered to the external circuit:

$$
P_{\text{ext}} = VI = I^2 R_L
$$
...
Conservation of energy requires $P_{\text{total}} = P_{\text{ext}} + P_{\text{int}}$.

The efficiency of the cell:

$$
\eta = \frac{P_{\text{ext}}}{P_{\text{total}}} = \frac{R_L}{R_L + r}
$$

<details>
<summary>Worked Example: Power Transfer</summary>

A battery with emf $12\text{ V}$ and internal resistance $2.0\,\Omega$ is connected to an external
resistor of $4.0\,\Omega$.

Current:

$$
I = \frac{\varepsilon}{R_L + r} = \frac{12}{4.0 + 2.0} = 2.0\text{ A}
$$

Power to external resistor:

$$
P_{\text{ext}} = I^2 R_L = (2.0)^2(4.0) = 16\text{ W}
$$

Power wasted internally:

$$
P_{\text{int}} = I^2 r = (2.0)^2(2.0) = 8\text{ W}
$$

Total power: $P_{\text{total}} = 16 + 8 = 24\text{ W}$.

Check: $P_{\text{total}} = \varepsilon I = 12 \times 2.0 = 24\text{ W}$.
...
$$
I_{\text{measured}} = \frac{\varepsilon}{R + R_A + r}
$$

which is less than the true current $I_{\text{true}} = \varepsilon/(R + r)$.
...
full-scale deflection current $I_g = 100\,\mu\text{A}$ and internal resistance $R_g = 50\,\Omega$.
...
For a galvanometer to measure up to $I = 1\text{ A}$:
...
For a galvanometer to measure up to $V = 10\text{ V}$:
$R_m = 10/(100 \times 10^{-6}) - 50 = 99950\,\Omega \approx 100\,\text{k}\Omega$.
...
$$
\varepsilon = \frac{E_{\text{chemical}}}{Q}
$$

For a cell that converts $E_{\text{chemical}}$ joules of chemical energy when $Q$ coulombs of charge
flow, the emf is $\varepsilon = E_{\text{chemical}}/Q$.
...
For a lead-acid battery, the emf drops from about $12.6\text{ V}$ (fully charged) to approximately
$11.9\text{ V}$ (discharged), while internal resistance can increase by an order of magnitude.
...
Two cells are connected in parallel across a common load. Cell 1 has $\varepsilon_1 = 12\text{ V}$,
$r_1 = 1.0\,\Omega$. Cell 2 has $\varepsilon_2 = 10\text{ V}$, $r_2 = 2.0\,\Omega$. The load
...
Then $I_1 = 12 - 5(2.0) = 2.0\text{ A}$ and $I_2 = 5 - 2.5(2.0) = 0\text{ A}$.

In this specific case, cell 2 contributes no current. The terminal p.d. of cell 2 is
$V_2 = \varepsilon_2 = 10\text{ V}$ (no current, so no internal drop), which equals the terminal
p.d. of cell 1: $V_1 = 12 - 2.0(1.0) = 10\text{ V}$. The p.d. across the load is
$V_L = I_L R_L = 2.0 \times 5.0 = 10\text{ V}$. This is consistent.
