...
$a_{\text{low}} \le t \le a_{\text{high}}$. If $a_{\text{mid}} \lt t$, then by monotonicity every
element at index $\le \text{mid}$ is also $\lt t$, so $k \gt \text{mid}$ and we safely set
$\text{low} = \text{mid} - 1$. The argument is symmetric for the other case. The loop terminates
when $\text{low} \gt
\text{high}$, meaning the search space is empty and $t$ does not exist in the
array.
