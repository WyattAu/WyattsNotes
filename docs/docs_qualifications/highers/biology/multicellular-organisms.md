...
1. Obtaining pluripotent stem cells (from embryos or by reprogramming adult cells).
2. Differentiating the stem cells into dopamine-producing neurones in the laboratory.
3. Transplanting these neurones into the patient's brain.
4. The transplanted neurones produce dopamine, reducing the symptoms of Parkinson's disease.
...
In 2006, Shinya Yamanaka discovered that adult somatic cells could be reprogrammed to become
pluripotent by introducing four transcription factors (Oct4, Sox2, Klf4, c-Myc) -- the "Yamanaka
factors." iPSCs behave like embryonic stem cells and can differentiate into any cell type. This has
enormous potential for regenerative medicine because it avoids the ethical issues associated with
embryonic stem cells and allows patient-specific therapies (using the patient's own cells).
...
**Resting potential:** Approximately $-70 \text{ mV}$ inside the axon relative to outside.
Maintained by the sodium-potassium pump ($3 \text{ Na}^+$ out, $2 \text{ K}^+$ in) and selective
permeability of the membrane.

**Action potential:**

1. Stimulus causes voltage-gated $\text{Na}^+$ channels to open
2. $\text{Na}^+$ rushes in, depolarising the membrane (to about $+40 \text{ mV}$)
3. Voltage-gated $\text{K}^+$ channels open
4. $\text{K}^+$ rushes out, repolarising the membrane
...
stimulated again. This ensures one-way transmission.

**Worked Example: Saltatory conduction.**

In a myelinated axon, the action potential "jumps" from one node of Ranvier to the next, rather than
propagating along the entire length of the axon membrane. This speeds up transmission because:
...
2. Voltage-gated $\text{Ca}^{2+}$ channels open; calcium ions enter
...
6. Ion channels open, causing depolarisation (excitatory) or hyperpolarisation (inhibitory)
7. Neurotransmitter is broken down by enzymes or reabsorbed (recycled)

**Example:** Acetylcholine is a common excitatory neurotransmitter. It is broken down by
acetylcholinesterase in the synaptic cleft.

**Worked Example: Spatial and temporal summation.**

A single excitatory postsynaptic potential (EPSP) is usually too small to reach the threshold for an
action potential (typically about $-55$ mV from a resting potential of $-70$ mV). However, if
...
The endocrine system uses chemical messengers (hormones) secreted by endocrine glands into the
bloodstream.
...
**Normal blood glucose:** Approximately $4-6 \text{ mmol/L}$.
...
1. **Pulmonary circulation:** Heart to lungs (deoxygenated) and lungs to heart (oxygenated)
2. **Systemic circulation:** Heart to body (oxygenated) and body to heart (deoxygenated)

**Cardiac cycle:**

1. **Atrial systole:** Atria contract, pushing blood into ventricles
2. **Ventricular systole:** Ventricles contract, pushing blood into aorta and pulmonary artery
3. **Diastole:** Heart muscle relaxes; blood flows into atria from veins

**Cardiac output:**

$$\text{Cardiac output} = \text{stroke volume} \times \text{heart rate}$$

**Example:** If stroke volume is $70 \text{ mL}$ and heart rate is $72 \text{ bpm}$:

$$\text{Cardiac output} = 70 \times 72 = 5040 \text{ mL/min} \approx 5.0 \text{ L/min}$$
...
1. **Action potential direction:** Depolarisation is caused by $\text{Na}^+$ influx; repolarisation
   by $\text{K}^+$ efflux.
...
1. Describe the sequence of events at a cholinergic synapse.

2. Explain how the structure of a motor neurone is adapted to its function.

3. Compare the nervous and endocrine systems as communication systems.

4. A person has a stroke volume of $80 \text{ mL}$ and a heart rate of $65 \text{ bpm}$. Calculate
   the cardiac output.

5. Describe how insulin and glucagon work together to maintain blood glucose homeostasis.

6. Explain how alveoli are adapted for efficient gas exchange, referring to Fick's Law.

7. Describe the role of the pancreas as both an endocrine and exocrine gland.

8. Explain the importance of the refractory period in the transmission of action potentials.

9. Explain why the myelin sheath increases the speed of nerve impulse transmission.
...
23. Describe the process of lipid digestion and absorption, explaining the role of bile.
...
The alveolar ventilation is lower because some air (approximately 150 mL) remains in the dead space
...
**Ultrafiltration:** In the glomerulus, high blood pressure forces water, glucose, amino acids,
urea, and ions out of the blood and into the Bowman's capsule. Large proteins and blood cells are
too large to pass through the basement membrane and remain in the blood.
...
medulla. The ascending limb is permeable to ions but not water; $\text{Na}^+$ and $\text{Cl}^-$ are
...
In a healthy person, all glucose is reabsorbed in the proximal convoluted tubule by active
transport. The transporters have a maximum capacity (transport maximum, Tm) of approximately 375
mg/min. If blood glucose levels are normal (approximately 5 mmol/L), the filtered load of glucose is
well below Tm, and all glucose is reabsorbed. In a person with uncontrolled diabetes, blood glucose
levels can exceed 10 mmol/L, and the filtered load exceeds Tm. The excess glucose cannot be
reabsorbed and appears in the urine (glycosuria).
...
**Spermatogenesis:** The process of sperm production in the seminiferous tubules. Spermatogonia
(diploid) undergo mitosis to produce more spermatogonia and primary spermatocytes. Primary
spermatocytes undergo meiosis I to produce secondary spermatocytes, which undergo meiosis II to
produce spermatids. Spermatids mature into spermatozoa (sperm) through a process called
spermiogenesis.

### Female Reproductive System

The female reproductive system produces eggs, provides a site for fertilisation, and supports fetal
development.
...
- Haemoglobin binds reversibly to oxygen: $\text{Hb} + 4\text{O}_2 \rightleftharpoons \text{HbO}_8$.
...
Small cell fragments involved in blood clotting. When a blood vessel is damaged, platelets
accumulate at the site and release clotting factors. This triggers the clotting cascade:

1. Thromboplastin is released from damaged tissue and platelets.
2. Thromboplastin converts prothrombin to thrombin (with calcium ions).
...
Calcium ions ($\text{Ca}^{2+}$) are essential cofactors at multiple steps in the clotting cascade.
...
The lymphatic system is a network of vessels, nodes, and organs that complements the circulatory
system.
...
bloodstream via the thoracic duct (drains the left side of the body and both legs) and the right
lymphatic duct (drains the right side of the body) into the subclavian veins.
...
- Contracts slowly and rhythmically; does not fatigue easily.
...
When a muscle contracts, the actin (thin) and myosin (thick) filaments slide past each other,
shortening the sarcomere. The process:

1. An action potential arrives at the neuromuscular junction, releasing acetylcholine.
2. The action potential spreads along the muscle fibre membrane and into T-tubules.
...
When the action potential ends, calcium is actively pumped back into the sarcoplasmic reticulum.
Troponin returns to its original shape, tropomyosin covers the binding sites, and the muscle
relaxes.
