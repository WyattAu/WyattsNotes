...
description:
  'Typescript Enums and Modules notes covering key definitions, core concepts, worked examples, and
...
```ts
import { add, Vector, PI } from './math';
```
...
import { Vector as Vec2D } from './math';
```
...
```ts
export { add as plus, subtract as minus } from './math';
```
