...
capacitor bank) smooths this into a relatively flat high-voltage DC bus, typically around 340 V for
...
500 W unit, not a 600 W unit. Reputable manufacturers clearly label both values.
...
PSU capacity through one wire before SCP triggers, but SCP is typically faster than the thermal
limit of the wire.

**Multi-rail 12V:** The +12 V output is split into two or more channels, each with independent OCP
trip points (typically 20--40 A per rail). This limits the maximum current on any single cable run,
...
units may or may not handle transients well — it depends on the bulk capacitor bank size, DC-DC
...
**Definition.** OPP shuts down the PSU when total output power exceeds a safe threshold, typically
...
**Definition.** OTP shuts down the PSU when internal temperatures exceed a safe threshold, typically
...
(shim-signed), so Secure Boot typically works out of the box. Custom kernels or unsigned bootloaders
...
The CMOS (Complementary Metal-Oxide-Semiconductor) battery — typically a CR2032 lithium coin cell —
...
AMI UEFI implementations are the most common. They typically use a left-hand navigation pane with
...
is also 100 MHz. On AM5, BCLK is typically 100 MHz but the architecture is more tolerant of BCLK
adjustments.
...
- **1:1 mode (FCLK = UCLK/2 = Memory/2):** For DDR4-3600, FCLK = 1800 MHz. This is the preferred
  mode for latency-sensitive workloads. FCLK can typically be pushed to 1800--1900 MHz on Zen 3 and
...
memory stability. Default is typically 0.95 V. Increasing to 1.0--1.05 V can improve memory
...
- **Tau:** The duration (in seconds) that the CPU can sustain PL2 before dropping to PL1. Typically
...
specified settings. Without enabling a profile, the memory runs at JEDEC standard speeds (typically
...
and precharging it. Must be at least tRCD + tRP, but is typically set higher for stability.
...
- **Gear 1:** Memory controller runs at the same frequency as the memory bus (1:1 ratio). Lower
  latency, best performance. Typically limited to DDR5-5600--6000 depending on the CPU's memory
  controller quality.
...
:::info For gaming and general use, Gear 1 at DDR5-6000 is typically the sweet spot. Gear 2 becomes
...
Performance gains vary by game and resolution. At 1080p, gains are typically 1--5%. At 1440p and 4K,
...
- **Auto:** BIOS typically selects the discrete GPU if one is detected, falling back to iGPU.
...
4. Insert the USB drive into the dedicated BIOS flashback USB port (usually marked with a specific
   label).
5. Press and hold the BIOS flashback button for 3--5 seconds. The LED indicator will flash.
6. Wait for the LED to stop flashing (typically 3--10 minutes). Do not power off during this
   process.

**Crisis recovery (Dell, HP, Lenovo):** OEM systems may require a different recovery procedure,
typically involving a special key combination during power-on and a recovery file on a USB drive.
...
2. **CMOS jumper:** Locate the CMOS jumper (usually labeled JBAT1, CLR_CMOS, or similar) on the
