...
`TIME-WAIT` and waits for $2 \times \text{MSL}$ (Maximum Segment Lifetime, typically 60 seconds per
...
$$
\text{RTT}_{\text{srtt}} = (1 - \alpha) \times \text{RTT}_{\text{srtt}} + \alpha \times \text{RTT}_{\text{latest}}
$$

$$
\text{RTT}_{\text{rttvar}} = (1 - \beta) \times \text{RTT}_{\text{rttvar}} + \beta \times |\text{RTT}_{\text{srtt}} - \text{RTT}_{\text{latest}}|
$$

$$
\text{RTO} = \text{RTT}_{\text{srtt}} + 4 \times \text{RTT}_{\text{rttvar}}
$$
...
$2^{\text{scale}}$, where scale ranges from 0 to 14:

$$
\text{Effective Window} = \text{Window} \times 2^{\text{scale factor}}
$$
...
For a 10 Gbps link with 100 ms RTT: $\text{BDP} = 10 \times 10^9 \times 0.1 = 10^9$ bytes (~953 MB).
...
$$
\text{Throughput} = \frac{\min(\text{rwnd}, \text{cwnd})}{\text{RTT}}
$$
...
increases by $\text{MSS} \times (\text{MSS} / \text{cwnd})$.
...
$\text{MSS} = 1500 - 20 (\text{IP}) - 20 (\text{TCP}) = 1460$ bytes.
...
4. **Misunderstanding `cwnd` and `rwnd`.** `cwnd` limits sending based on network congestion. `rwnd`
   ...
   $\min(\text{rwnd}, \text{cwnd}) / \text{RTT}$. A full receiver buffer (`rwnd = 0`) stops the
