...
<<<<<<< Updated
upstream:docs/docs_dse/ict/3-programming-and-databases/2_programming-and-databases.md

### Example 1: SQL query

**Problem.** A table Students(ID, Name, Grade, Age). Write SQL to find all students with Grade
$> 80$, ordered by Age.

**Solution.** `SELECT Name, Grade FROM Students WHERE Grade > 80 ORDER BY Age;`

$\blacksquare$

### Example 2: Normalisation

**Problem.** A table has records: (OrderID, CustomerName, CustomerAddress, Product, Quantity).
Identify normalisation issues.

**Solution.** The table violates 1NF (CustomerAddress may not be atomic), 2NF (CustomerName depends
on OrderID, not on the full key), and 3NF. Split into: Customers(CustomerID, Name, Address),
Orders(OrderID, CustomerID), OrderDetails(OrderID, Product, Quantity).

$\blacksquare$

## Summary

- Relational databases: tables with primary keys and foreign keys; ensure data integrity.
- SQL: SELECT (retrieve), INSERT (add), UPDATE (modify), DELETE (remove); WHERE, ORDER BY, JOIN.
...
A typical problem in ict - programming and databases involves identifying the relevant chemical
concepts, writing appropriate equations, and performing calculations with correct units and
significant figures.
...
4. Give the final answer with appropriate units and significant figures
   > > > > > > > Stashed changes:docs/docs_dse/ICT/programming-and-databases.md
