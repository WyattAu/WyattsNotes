...
categories:
  - Chemistry
slug: dse-chemistry-atomic-structure-and-periodic-table
---
...
The periodic table arranges elements in order of increasing atomic number. Elements in the same
**group** (column) have similar chemical properties because they have the same number of valence
electrons. Elements in the same **period** (row) have the same number of electron shells.
...
Each subsequent ionisation energy is larger because the electron is removed from an increasingly
positive ion. A large jump in successive ionisation energies indicates the removal of an electron
...
Transition metals are d-block elements that form at least one stable ion with an incomplete d
subshell. Key properties:
