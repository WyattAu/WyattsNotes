...
categories:
  - Chemistry
slug: dse-chemistry-acids-bases-and-salts
---
...
A base is a substance that neutralises an acid. A **soluble base** is called an **alkali** and
...
A strong acid is completely dissociated in aqueous solution. The equilibrium lies entirely to the
right:
...
The method used to prepare a salt depends on the solubility of the salt and the reactivity of the
metal or base.
...
For metals that are not reactive enough to displace hydrogen (e.g., copper), or for metal oxides and
hydroxides:
...
The exact volume of alkali needed to neutralise the acid is found by titration. The solution is then
...
Describe how to prepare pure, dry crystals of copper(II) sulphate from copper(II) oxide and dilute
sulphuric acid.
...
Ionic equations show only the species that actually participate in a reaction (spectator ions are
cancelled).
...
Bubble the gas through limewater ($\mathrm{Ca(OH)_2}$ solution). If $\mathrm{CO_2}$ is present, the
...
$5.00 \mathrm{ g}$ of calcium carbonate is reacted with excess hydrochloric acid. Calculate the
...
Group 1 carbonates (except lithium carbonate) do not decompose on heating. Other metal carbonates
decompose:
...
**Question 4:** $2.50 \mathrm{ g}$ of $\mathrm{CaCO_3}$ is added to $100 \mathrm{ cm^3}$ of
$1.00 \mathrm{ mol/dm^3}$ $\mathrm{HCl}$. Calculate the volume of $\mathrm{CO_2}$ produced at RTP.
