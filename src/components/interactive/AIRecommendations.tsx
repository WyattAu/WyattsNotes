...
const FALLBACK: Record<string, string[]> = {
  physics: ['1_kinematics', '1_thermodynamics', '2_wave-properties', '1_gravitational-fields', '1_quantum-and-nuclear-physics'],
  biology: ['1_cell-biology', '1_molecular-biology', '1_genetics', '1_ecology', '1_evolution'],
  chemistry: [],
  economics: ['1_supply-and-demand', '2_elasticity', '3_market-failure', '1_national-income', '1_game-theory-and-behavioural'],
  psychology: ['memory-models', 'attachment-theory', 'brain-imaging-techniques', 'depression', 'social-identity-theory'],
  geography: ['demographic-transition-model', 'drainage-basins-and-hydrology', 'atmospheric-systems', 'urbanisation-trends-and-patterns', 'measuring-development'],
};
...
function getReasonLabel(score: number): string {
  if (score > 0.3) return 'Highly related content';
  if (score > 0.15) return 'Related topic';
  return 'Tangentially related';
}

export function AIRecommendations({ currentSlug, category, maxResults = 5 }: AIRecommendationsProps) {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/search?${params}`)
      .then(r => r.json())
      .then(data => setRecommendations((data.results || []).filter((r: Recommendation) => r.slug !== currentSlug).slice(0, maxResults)))
      .catch(() => setError(true))
  ...
  if (!visible) return <div ref={containerRef} style={{ minHeight: '20px' }} />;
  ...
  const styles: Record<string, React.CSSProperties> = {
    ...
    card: {
      border: '1px solid #e8e8e8', borderRadius: '8px', padding: '16px', background: '#fafafa',
    ...
    reason: { fontSize: '12px', color: '#3498db', fontStyle: 'italic', marginBottom: '8px', display: expandedSlug ? 'block' : 'none' },
    actions: { display: 'flex', gap: '8px' },
    btnSmall: {
      fontSize: '11px', border: '1px solid #ddd', borderRadius: '4px', padding: '2px 8px',
      background: '#fff', color: '#666', cursor: 'pointer',
    },
    loading: { color: '#888', fontSize: '14px' },
    fallbackCard: {
      border: '1px solid #e8e8e8', borderRadius: '8px', padding: '12px 16px', background: '#f0f8ff',
      fontSize: '13px', color: '#2c3e50',
    },
  };
  ...
  return (
    <div ref={containerRef} style={styles.container}>
        ...
        <div>
          ...
          <div style={styles.grid}>
            {fallbackList.slice(0, maxResults).map(slug => (
              <a key={slug} href={slug} style={styles.fallbackCard}>{slug.replace(/_/g, ' ')}</a>
            ))}
          </div>
        </div>
      )}
        ...
        <div style={styles.grid}>
          {shown.map(r => (
            <div key={r.slug} style={styles.card} className="ai-rec-card">
              ...
              <div style={{ ...styles.reason, display: expandedSlug === r.slug ? 'block' : 'none' }}>
                {getReasonLabel(r.score)}: This topic shares concepts with the page you are viewing.
              </div>
              <div style={styles.actions}>
                <button style={styles.btnSmall} onClick={() => setExpandedSlug(expandedSlug === r.slug ? null : r.slug)}>
                  {expandedSlug === r.slug ? 'Hide' : 'Why this?'}
                </button>
                <button style={styles.btnSmall} onClick={() => handleDismiss(r.slug)}>Not relevant</button>
              </div>
            </div>
          ))}
        </div>
      ...
      {!error && !loading && shown.length === 0 && (
        <div style={{ fontSize: '13px', color: '#888' }}>No recommendations available for this page.</div>
      )}
    </div>
  );
}
